...
class Db extends Think
{
    ...
    public function connect() {
        if(!$this->connected) {
            ...
            if($this->pconnect) {
                $this->linkID = mysqli_pconnect( $host, $config['username'], $config['password'],CLIENT_MULTI_RESULTS);
            }else{
                $this->linkID = mysqli_connect( $host, $config['username'], $config['password'],true,CLIENT_MULTI_RESULTS);
            }
            if ( !$this->linkID || (!empty($config['database']) && !mysqli_select_db($config['database'], $this->linkID)) ) {
                throw_exception(mysqli_error());
            }
            $dbVersion = mysqli_get_server_info($this->linkID);
            if ($dbVersion >= "4.1") {
                //使用UTF8存取数据库 需要mysql 4.1.0以上支持
                mysqli_query("SET NAMES '".C('DB_CHARSET')."'", $this->linkID);
            }
            //设置 sql_model
            if($dbVersion >'5.0.1'){
    ...
    public function query($str='') {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $this->queryID = mysqli_query($this->queryStr, $this->linkID);
        $this->debug();
        if ( !$this->queryID ) {
            if ( $this->debug )
    ...
    public function execute($str='') {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $result =   mysqli_query($this->queryStr, $this->linkID) ;
        $this->debug();
        if ( false === $result) {
            if ( $this->debug )
                throw_exception($this->error());
    ...
    public function startTrans() {
        ...
        if ($this->transTimes == 0) {
            mysqli_query('START TRANSACTION', $this->linkID);
        }
        $this->transTimes++;
        return ;
    }
    ...
    {
        if ($this->transTimes > 0) {
            $result = mysqli_query('COMMIT', $this->linkID);
            $this->transTimes = 0;
            if(!$result){
                throw_exception($this->error());
                return false;
    ...
    {
        if ($this->transTimes > 0) {
            $result = mysqli_query('ROLLBACK', $this->linkID);
            $this->transTimes = 0;
            if(!$result){
                throw_exception($this->error());
                return false;
