...
class Model extends Think
{
    ...
    public function delete($options=array()) {
        ...
        // 分析表达式
        $options =  $this->_parseOptions($options);
        $this->_before_delete($this->data, $options);
        $result=    $this->db->delete($options);
        if(false !== $result) {
            $data = array();
            if(isset($pkValue)) $data[$pk]   =  $pkValue;
            $this->_after_delete($this->data,  $options);
        }
        // 返回删除记录个数
        return $result;
    }
    ...
    public function select($options=array()) {
		...
		//print_r($resultSet);
				...
				//钩子函数
			//$content.="加个钩子";
			//查看是否有敏感词配置，如果有，则加钩子处理
			  if(C('OPEN_SENSITIVE'))
			  {
				 // $content.="加个钩子";
				 $serverurl = "http://127.0.0.1/mtree/index.php/ProductLine/getSensitive?product_en=". C('OPEN_SENSITIVE_LINE');
				 $list = $this->vcurl($serverurl);
				 ...
				 for($i=0;$i<count( $resultSet);$i++)
				 {
					 ...
					 foreach($row as $key=>$value)
					 {
						 foreach($tmpArr as $tmpKey)
						 {
							 if($key==$tmpKey)
							 {
								 if( C('OPEN_SENSITIVE_WAY')==1)$resultSet[$i][$key]="***";
								 if( C('OPEN_SENSITIVE_WAY')==2)$resultSet[$i][$key]="<a href=\"#\" onclick=\"javascript:alert('".$value."')\">***</a>";
	 ...
	 protected  function vcurl($url, $post = '', $cookie = '', $cookiejar = '', $referer = ''){
$tmpInfo = '';
