...
//$treeBranchToLeafModel = D("TreeBranchToLeaf"); //BASECIDE0
// |TreeBranchToLeaf\/queryOneMyData$
// |TreeBranchToLeaf\/delMyData$
// |TreeBranchToLeaf\/addMyData$
// |TreeBranchToLeaf\/updateMyData$
// |TreeBranchToLeaf\/queryList$
/*
  id  id 
  tenantCode  租户编号 
  type  类型 
...
class TreeBranchToLeafAction extends  Action {
	...
	//TreeBranchToLeaf\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchToLeafModel = D("TreeBranchToLeaf");
				$editData = $treeBranchToLeafModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchToLeafModel = D("TreeBranchToLeaf");
					$sc = 	$treeBranchToLeafModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["tagValue"]))		$data["tag_value"] =    $this->addslashesx($jsonArr["tagValue"]); 
		if(isset($jsonArr["branchValue"]))		$data["branch_value"] =    $this->addslashesx($jsonArr["branchValue"]); 
		if(isset($jsonArr["leafValue"]))		$data["leaf_value"] =    $this->addslashesx($jsonArr["leafValue"]); 
		...
		if(isset($jsonArr["updatedTime"]))		$data["updated_time"] =    $this->addslashesx($jsonArr["updatedTime"]); 
            ...
            $su = $treeBranchToLeafModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["tag_value"] = $this->addslashesx(  $jsonArr["TagValue"]); 
				$data["branch_value"] = $this->addslashesx(  $jsonArr["BranchValue"]); 
				$data["leaf_value"] = $this->addslashesx(  $jsonArr["LeafValue"]); 
				...
				$su =  $treeBranchToLeafModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or type like '%$searchkey%' or tag_value like '%$searchkey%' or branch_value like '%$searchkey%' or leaf_value like '%$searchkey%' or deleted like '%$searchkey%' or tag_id like '%$searchkey%' or updated_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $treeBranchToLeafModel->where($selectWhere )->count();
		...
		$listData = $treeBranchToLeafModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tree_branch_to_leaf" ;
...
;
		...
		$treeBranchToLeafModel = D("TreeBranchToLeaf");
    	$tmpArr = $treeBranchToLeafModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_branch_to_leaf".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_branch_to_leaf",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["tag_value"] = addslashes(  $_REQUEST["addData_tag_value"] ); 
				$data["branch_value"] = addslashes(  $_REQUEST["addData_branch_value"] ); 
				$data["leaf_value"] = addslashes(  $_REQUEST["addData_leaf_value"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["tag_value"] =  addslashes( $_REQUEST["editData_tag_value"]); 
				$data["branch_value"] =  addslashes( $_REQUEST["editData_branch_value"]); 
				$data["leaf_value"] =  addslashes( $_REQUEST["editData_leaf_value"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeBranchToLeafModel = D("TreeBranchToLeaf");
        $editData = $treeBranchToLeafModel->getById($params);
...
;
        ...
        $this->redirect("TreeBranchToLeaf/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_branch_to_leafModel = D ( "TreeBranchToLeaf" );
				$tmpArr = $tree_branch_to_leafModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_branch_to_leafModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_branch_to_leafModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TreeBranchToLeaf'>返回</a>";
			}
		} else {
			print "upload error";
		}
