...
//$filterTagCopynumberModel = D("FilterTagCopynumber"); //BASECIDE0
// |FilterTagCopynumber\/queryOneMyData$
// |FilterTagCopynumber\/delMyData$
// |FilterTagCopynumber\/addMyData$
// |FilterTagCopynumber\/updateMyData$
// |FilterTagCopynumber\/queryList$
/**/

class FilterTagCopynumberAction extends  Action {
	...
	//FilterTagCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterTagCopynumberModel = D("FilterTagCopynumber");
				$editData = $filterTagCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterTagCopynumberModel = D("FilterTagCopynumber");
					$sc = 	$filterTagCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $filterTagCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $filterTagCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $filterTagCopynumberModel->where($selectWhere )->count();
		...
		$listData = $filterTagCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."filter_tag_copynumber" ;
...
;
		...
		$filterTagCopynumberModel = D("FilterTagCopynumber");
    	$tmpArr = $filterTagCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "filter_tag_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"filter_tag_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$filterTagCopynumberModel = D("FilterTagCopynumber");
        $editData = $filterTagCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("FilterTagCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$filter_tag_copynumberModel = D ( "FilterTagCopynumber" );
				$tmpArr = $filter_tag_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$filter_tag_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $filter_tag_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../FilterTagCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
