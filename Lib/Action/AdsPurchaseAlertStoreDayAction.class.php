...
//$adsPurchaseAlertStoreDayModel = D("AdsPurchaseAlertStoreDay"); //BASECIDE0
// |AdsPurchaseAlertStoreDay\/queryOneMyData$
// |AdsPurchaseAlertStoreDay\/delMyData$
// |AdsPurchaseAlertStoreDay\/addMyData$
// |AdsPurchaseAlertStoreDay\/updateMyData$
// |AdsPurchaseAlertStoreDay\/queryList$
/*
  id  id 
  tenantCode  租户code 
  storeId  鱼店宝授权码 
...
class AdsPurchaseAlertStoreDayAction extends  Action {
	...
	//AdsPurchaseAlertStoreDay\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$adsPurchaseAlertStoreDayModel = D("AdsPurchaseAlertStoreDay");
				$editData = $adsPurchaseAlertStoreDayModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$adsPurchaseAlertStoreDayModel = D("AdsPurchaseAlertStoreDay");
					$sc = 	$adsPurchaseAlertStoreDayModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["difference"]))		$data["difference"] =    $this->addslashesx($jsonArr["difference"]); 
		if(isset($jsonArr["differencePercent"]))		$data["difference_percent"] =    $this->addslashesx($jsonArr["differencePercent"]); 
		if(isset($jsonArr["dt"]))		$data["dt"] =    $this->addslashesx($jsonArr["dt"]); 

            $su = $adsPurchaseAlertStoreDayModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["difference"] = $this->addslashesx(  $jsonArr["Difference"]); 
				$data["difference_percent"] = $this->addslashesx(  $jsonArr["DifferencePercent"]); 
				$data["dt"] = $this->addslashesx(  $jsonArr["Dt"]); 



				$su =  $adsPurchaseAlertStoreDayModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or store_id like '%$searchkey%' or task_id like '%$searchkey%' or period like '%$searchkey%' or materia like '%$searchkey%' or tenant_type like '%$searchkey%' or warehouse_name like '%$searchkey%' or sub_company like '%$searchkey%' or agent like '%$searchkey%' or store_name like '%$searchkey%' or province like '%$searchkey%' or city like '%$searchkey%' or county like '%$searchkey%' or yl like '%$searchkey%' or jh_cnt like '%$searchkey%' or is_alert like '%$searchkey%' or alert_reason like '%$searchkey%' or difference like '%$searchkey%' or difference_percent like '%$searchkey%' or dt like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $adsPurchaseAlertStoreDayModel->where($selectWhere )->count();
		...
		$listData = $adsPurchaseAlertStoreDayModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."ads_purchase_alert_store_day" ;
...
;
		...
		$adsPurchaseAlertStoreDayModel = D("AdsPurchaseAlertStoreDay");
    	$tmpArr = $adsPurchaseAlertStoreDayModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "ads_purchase_alert_store_day".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"ads_purchase_alert_store_day",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["difference"] = addslashes(  $_REQUEST["addData_difference"] ); 
				$data["difference_percent"] = addslashes(  $_REQUEST["addData_difference_percent"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["difference"] =  addslashes( $_REQUEST["editData_difference"]); 
				$data["difference_percent"] =  addslashes( $_REQUEST["editData_difference_percent"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$adsPurchaseAlertStoreDayModel = D("AdsPurchaseAlertStoreDay");
        $editData = $adsPurchaseAlertStoreDayModel->getById($params);
...
;
        ...
        $this->redirect("AdsPurchaseAlertStoreDay/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$ads_purchase_alert_store_dayModel = D ( "AdsPurchaseAlertStoreDay" );
				$tmpArr = $ads_purchase_alert_store_dayModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$ads_purchase_alert_store_dayModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $ads_purchase_alert_store_dayModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../AdsPurchaseAlertStoreDay'>返回</a>";
			}
		} else {
			print "upload error";
		}
