...
//$tbDataassetsStatisticsExtendModel = D("TbDataassetsStatisticsExtend"); //BASECIDE0
// |TbDataassetsStatisticsExtend\/queryOneMyData$
// |TbDataassetsStatisticsExtend\/delMyData$
// |TbDataassetsStatisticsExtend\/addMyData$
// |TbDataassetsStatisticsExtend\/updateMyData$
// |TbDataassetsStatisticsExtend\/queryList$
/*
  id  id 
  tenantCode  租户code 
  bizTag  供应链scm 
...
class TbDataassetsStatisticsExtendAction extends  Action {
	...
	//TbDataassetsStatisticsExtend\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbDataassetsStatisticsExtendModel = D("TbDataassetsStatisticsExtend");
				$editData = $tbDataassetsStatisticsExtendModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbDataassetsStatisticsExtendModel = D("TbDataassetsStatisticsExtend");
					$sc = 	$tbDataassetsStatisticsExtendModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updateTime"]))		$data["update_time"] =    $this->addslashesx($jsonArr["updateTime"]); 
            ...
            $su = $tbDataassetsStatisticsExtendModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $tbDataassetsStatisticsExtendModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or biz_tag like '%$searchkey%' or table_name like '%$searchkey%' or year_str like '%$searchkey%' or storage like '%$searchkey%' or number like '%$searchkey%' or update_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tbDataassetsStatisticsExtendModel->where($selectWhere )->count();
		...
		$listData = $tbDataassetsStatisticsExtendModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_dataassets_statistics_extend" ;
...
;
		...
		$tbDataassetsStatisticsExtendModel = D("TbDataassetsStatisticsExtend");
    	$tmpArr = $tbDataassetsStatisticsExtendModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_dataassets_statistics_extend".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_dataassets_statistics_extend",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbDataassetsStatisticsExtendModel = D("TbDataassetsStatisticsExtend");
        $editData = $tbDataassetsStatisticsExtendModel->getById($params);
...
;
        ...
        $this->redirect("TbDataassetsStatisticsExtend/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_dataassets_statistics_extendModel = D ( "TbDataassetsStatisticsExtend" );
				$tmpArr = $tb_dataassets_statistics_extendModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_dataassets_statistics_extendModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_dataassets_statistics_extendModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TbDataassetsStatisticsExtend'>返回</a>";
			}
		} else {
			print "upload error";
		}
