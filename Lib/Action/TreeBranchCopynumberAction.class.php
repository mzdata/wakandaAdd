...
//$treeBranchCopynumberModel = D("TreeBranchCopynumber"); //BASECIDE0
// |TreeBranchCopynumber\/queryOneMyData$
// |TreeBranchCopynumber\/delMyData$
// |TreeBranchCopynumber\/addMyData$
// |TreeBranchCopynumber\/updateMyData$
// |TreeBranchCopynumber\/queryList$
/**/

class TreeBranchCopynumberAction extends  Action {
	...
	//TreeBranchCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchCopynumberModel = D("TreeBranchCopynumber");
				$editData = $treeBranchCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchCopynumberModel = D("TreeBranchCopynumber");
					$sc = 	$treeBranchCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $treeBranchCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $treeBranchCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $treeBranchCopynumberModel->where($selectWhere )->count();
		...
		$listData = $treeBranchCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tree_branch_copynumber" ;
...
;
		...
		$treeBranchCopynumberModel = D("TreeBranchCopynumber");
    	$tmpArr = $treeBranchCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_branch_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_branch_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeBranchCopynumberModel = D("TreeBranchCopynumber");
        $editData = $treeBranchCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("TreeBranchCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_branch_copynumberModel = D ( "TreeBranchCopynumber" );
				$tmpArr = $tree_branch_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_branch_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_branch_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TreeBranchCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
