...
//$sqlTemplateVnumberModel = D("SqlTemplateVnumber"); //BASECIDE0
// |SqlTemplateVnumber\/queryOneMyData$
// |SqlTemplateVnumber\/delMyData$
// |SqlTemplateVnumber\/addMyData$
// |SqlTemplateVnumber\/updateMyData$
// |SqlTemplateVnumber\/queryList$
/**/

class SqlTemplateVnumberAction extends  Action {
	...
	//SqlTemplateVnumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqlTemplateVnumberModel = D("SqlTemplateVnumber");
				$editData = $sqlTemplateVnumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqlTemplateVnumberModel = D("SqlTemplateVnumber");
					$sc = 	$sqlTemplateVnumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $sqlTemplateVnumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $sqlTemplateVnumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $sqlTemplateVnumberModel->where($selectWhere )->count();
		...
		$listData = $sqlTemplateVnumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."sql_template_vnumber" ;
...
;
		...
		$sqlTemplateVnumberModel = D("SqlTemplateVnumber");
    	$tmpArr = $sqlTemplateVnumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sql_template_vnumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sql_template_vnumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$sqlTemplateVnumberModel = D("SqlTemplateVnumber");
        $editData = $sqlTemplateVnumberModel->getById($params);
...
;
        ...
        $this->redirect("SqlTemplateVnumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sql_template_vnumberModel = D ( "SqlTemplateVnumber" );
				$tmpArr = $sql_template_vnumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sql_template_vnumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sql_template_vnumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../SqlTemplateVnumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
