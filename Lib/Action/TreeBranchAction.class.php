...
//$treeBranchModel = D("TreeBranch"); //BASECIDE0
// |TreeBranch\/queryOneMyData$
// |TreeBranch\/delMyData$
// |TreeBranch\/addMyData$
// |TreeBranch\/updateMyData$
// |TreeBranch\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  type  类型 
...
class TreeBranchAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchModel = D("TreeBranch");
				$editData = $treeBranchModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchModel = D("TreeBranch");
					$sc = 	$treeBranchModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["parentValue"]))		$data["parent_value"] =    $this->addslashesx($jsonArr["parentValue"]); 
		...
		if(isset($jsonArr["tagId"]))		$data["tag_id"] =    $this->addslashesx($jsonArr["tagId"]); 
            ...
            $su = $treeBranchModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["parent_value"] = $this->addslashesx(  $jsonArr["ParentValue"]); 
				...
				$su =  $treeBranchModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or type like '%$searchkey%' or tag_value like '%$searchkey%' or name like '%$searchkey%' or value like '%$searchkey%' or parent_value like '%$searchkey%' or deleted like '%$searchkey%' or update_time like '%$searchkey%' or tag_id like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $treeBranchModel->where($selectWhere )->count();
		...
		$listData = $treeBranchModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$treeBranchModel = D("TreeBranch");
    	$tmpArr = $treeBranchModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_branch".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_branch",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["parent_value"] = addslashes(  $_REQUEST["addData_parent_value"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["parent_value"] =  addslashes( $_REQUEST["editData_parent_value"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeBranchModel = D("TreeBranch");
        $editData = $treeBranchModel->getById($params);
...
;
        ...
        $this->redirect("TreeBranch/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_branchModel = D ( "TreeBranch" );
				$tmpArr = $tree_branchModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_branchModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_branchModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
