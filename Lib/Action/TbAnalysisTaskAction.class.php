...
//$tbAnalysisTaskModel = D("TbAnalysisTask"); //BASECIDE0
// |TbAnalysisTask\/queryOneMyData$
// |TbAnalysisTask\/delMyData$
// |TbAnalysisTask\/addMyData$
// |TbAnalysisTask\/updateMyData$
// |TbAnalysisTask\/queryList$
/*
  id  id 
  tenantCode  租户code 
  taskName  任务名 
...
class TbAnalysisTaskAction extends  Action {
	...
	//TbAnalysisTask\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbAnalysisTaskModel = D("TbAnalysisTask");
				$editData = $tbAnalysisTaskModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbAnalysisTaskModel = D("TbAnalysisTask");
					$sc = 	$tbAnalysisTaskModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updateTime"]))		$data["update_time"] =    $this->addslashesx($jsonArr["updateTime"]); 
            ...
            $su = $tbAnalysisTaskModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $tbAnalysisTaskModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or task_name like '%$searchkey%' or status like '%$searchkey%' or create_time like '%$searchkey%' or update_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tbAnalysisTaskModel->where($selectWhere )->count();
		...
		$listData = $tbAnalysisTaskModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_analysis_task" ;
...
;
		...
		$tbAnalysisTaskModel = D("TbAnalysisTask");
    	$tmpArr = $tbAnalysisTaskModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_analysis_task".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_analysis_task",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbAnalysisTaskModel = D("TbAnalysisTask");
        $editData = $tbAnalysisTaskModel->getById($params);
...
;
        ...
        $this->redirect("TbAnalysisTask/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_analysis_taskModel = D ( "TbAnalysisTask" );
				$tmpArr = $tb_analysis_taskModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_analysis_taskModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_analysis_taskModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TbAnalysisTask'>返回</a>";
			}
		} else {
			print "upload error";
		}
