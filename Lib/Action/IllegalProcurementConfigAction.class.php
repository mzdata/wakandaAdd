...
//$illegalProcurementConfigModel = D("IllegalProcurementConfig"); //BASECIDE0
// |IllegalProcurementConfig\/queryOneMyData$
// |IllegalProcurementConfig\/delMyData$
// |IllegalProcurementConfig\/addMyData$
// |IllegalProcurementConfig\/updateMyData$
// |IllegalProcurementConfig\/queryList$
/*
  id  id 
  tenantCode  租户 
  period  采样周期 
...
class IllegalProcurementConfigAction extends  Action {
	...
	//IllegalProcurementConfig\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$illegalProcurementConfigModel = D("IllegalProcurementConfig");
				$editData = $illegalProcurementConfigModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$illegalProcurementConfigModel = D("IllegalProcurementConfig");
					$sc = 	$illegalProcurementConfigModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updateTime"]))		$data["update_time"] =    $this->addslashesx($jsonArr["updateTime"]); 
            ...
            $su = $illegalProcurementConfigModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $illegalProcurementConfigModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $illegalProcurementConfigModel->where($selectWhere )->count();
		...
		$listData = $illegalProcurementConfigModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."illegal_procurement_config" ;
...
;
		...
		$illegalProcurementConfigModel = D("IllegalProcurementConfig");
    	$tmpArr = $illegalProcurementConfigModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "illegal_procurement_config".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"illegal_procurement_config",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$illegalProcurementConfigModel = D("IllegalProcurementConfig");
        $editData = $illegalProcurementConfigModel->getById($params);
...
;
        ...
        $this->redirect("IllegalProcurementConfig/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$illegal_procurement_configModel = D ( "IllegalProcurementConfig" );
				$tmpArr = $illegal_procurement_configModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$illegal_procurement_configModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $illegal_procurement_configModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../IllegalProcurementConfig'>返回</a>";
			}
		} else {
			print "upload error";
		}
