...
//$tmpCleaningSkuModel = D("TmpCleaningSku"); //BASECIDE0
// |TmpCleaningSku\/queryOneMyData$
// |TmpCleaningSku\/delMyData$
// |TmpCleaningSku\/addMyData$
// |TmpCleaningSku\/updateMyData$
// |TmpCleaningSku\/queryList$
/*
  id  id 
  commodityId  名字加规格 
  tenantCode  租户编号 
...
class TmpCleaningSkuAction extends  Action {
	...
	//TmpCleaningSku\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tmpCleaningSkuModel = D("TmpCleaningSku");
				$editData = $tmpCleaningSkuModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tmpCleaningSkuModel = D("TmpCleaningSku");
					$sc = 	$tmpCleaningSkuModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["cleaningSpec"]))		$data["cleaning_spec"] =    $this->addslashesx($jsonArr["cleaningSpec"]); 
            ...
            $su = $tmpCleaningSkuModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $tmpCleaningSkuModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $tmpCleaningSkuModel->where($selectWhere )->count();
		...
		$listData = $tmpCleaningSkuModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tmp_cleaning_sku" ;
...
;
		...
		$tmpCleaningSkuModel = D("TmpCleaningSku");
    	$tmpArr = $tmpCleaningSkuModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tmp_cleaning_sku".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tmp_cleaning_sku",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tmpCleaningSkuModel = D("TmpCleaningSku");
        $editData = $tmpCleaningSkuModel->getById($params);
...
;
        ...
        $this->redirect("TmpCleaningSku/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tmp_cleaning_skuModel = D ( "TmpCleaningSku" );
				$tmpArr = $tmp_cleaning_skuModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tmp_cleaning_skuModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tmp_cleaning_skuModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TmpCleaningSku'>返回</a>";
			}
		} else {
			print "upload error";
		}
