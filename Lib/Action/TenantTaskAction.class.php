...
//$tenantTaskModel = D("TenantTask"); //BASECIDE0
// |TenantTask\/queryOneMyData$
// |TenantTask\/delMyData$
// |TenantTask\/addMyData$
// |TenantTask\/updateMyData$
// |TenantTask\/queryList$
/*
  id  id 
  tenantName  租户名称 
  tenantCode  租户编码 
...
class TenantTaskAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tenantTaskModel = D("TenantTask");
				$editData = $tenantTaskModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tenantTaskModel = D("TenantTask");
					$sc = 	$tenantTaskModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["tenantName"]))		$data["tenant_name"] =    $this->addslashesx($jsonArr["tenantName"]); 
		...
		if(isset($jsonArr["taskName"]))		$data["task_name"] =    $this->addslashesx($jsonArr["taskName"]); 
		if(isset($jsonArr["cronTime"]))		$data["cron_time"] =    $this->addslashesx($jsonArr["cronTime"]); 
		if(isset($jsonArr["ifValid"]))		$data["if_valid"] =    $this->addslashesx($jsonArr["ifValid"]); 

            $su = $tenantTaskModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
 				...
 				$data = array();
									...
									$data["tenant_name"] = $this->addslashesx(  $jsonArr["TenantName"]); 
				$data["tenant_code"] = $this->addslashesx(  $jsonArr["TenantCode"]); 
				$data["type"] = $this->addslashesx(  $jsonArr["Type"]); 
				$data["task_name"] = $this->addslashesx(  $jsonArr["TaskName"]); 
				$data["cron_time"] = $this->addslashesx(  $jsonArr["CronTime"]); 
				$data["if_valid"] = $this->addslashesx(  $jsonArr["IfValid"]); 



				$su =  $tenantTaskModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_name like '%$searchkey%' or tenant_code like '%$searchkey%' or type like '%$searchkey%' or task_name like '%$searchkey%' or cron_time like '%$searchkey%' or if_valid like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tenantTaskModel->where($selectWhere )->count();
		...
		$listData = $tenantTaskModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$tenantTaskModel = D("TenantTask");
    	$tmpArr = $tenantTaskModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tenant_task".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tenant_task",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["tenant_name"] = addslashes(  $_REQUEST["addData_tenant_name"] ); 
				$data["tenant_code"] = addslashes(  $_REQUEST["addData_tenant_code"] ); 
				$data["type"] = addslashes(  $_REQUEST["addData_type"] ); 
				$data["task_name"] = addslashes(  $_REQUEST["addData_task_name"] ); 
				$data["cron_time"] = addslashes(  $_REQUEST["addData_cron_time"] ); 
				$data["if_valid"] = addslashes(  $_REQUEST["addData_if_valid"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["tenant_name"] =  addslashes( $_REQUEST["editData_tenant_name"]); 
				$data["tenant_code"] =  addslashes( $_REQUEST["editData_tenant_code"]); 
				$data["type"] =  addslashes( $_REQUEST["editData_type"]); 
				$data["task_name"] =  addslashes( $_REQUEST["editData_task_name"]); 
				$data["cron_time"] =  addslashes( $_REQUEST["editData_cron_time"]); 
				$data["if_valid"] =  addslashes( $_REQUEST["editData_if_valid"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tenantTaskModel = D("TenantTask");
        $editData = $tenantTaskModel->getById($params);
...
;
        ...
        $this->redirect("TenantTask/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tenant_taskModel = D ( "TenantTask" );
				$tmpArr = $tenant_taskModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tenant_taskModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tenant_taskModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
