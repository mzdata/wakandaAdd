...
//$tDownloadColumnModel = D("TDownloadColumn"); //BASECIDE0
// |TDownloadColumn\/queryOneMyData$
// |TDownloadColumn\/delMyData$
// |TDownloadColumn\/addMyData$
// |TDownloadColumn\/updateMyData$
// |TDownloadColumn\/queryList$
/*
  id  id 
  catalogEnum  分类名 
  tableHeaderName  表头 
...
class TDownloadColumnAction extends  Action {
	...
	//TDownloadColumn\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tDownloadColumnModel = D("TDownloadColumn");
				$editData = $tDownloadColumnModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tDownloadColumnModel = D("TDownloadColumn");
					$sc = 	$tDownloadColumnModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["tableHeaderName"]))		$data["table_header_name"] =    $this->addslashesx($jsonArr["tableHeaderName"]); 
		if(isset($jsonArr["fieldName"]))		$data["field_name"] =    $this->addslashesx($jsonArr["fieldName"]); 
		if(isset($jsonArr["num"]))		$data["num"] =    $this->addslashesx($jsonArr["num"]); 
		if(isset($jsonArr["tag"]))		$data["tag"] =    $this->addslashesx($jsonArr["tag"]); 

            $su = $tDownloadColumnModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["table_header_name"] = $this->addslashesx(  $jsonArr["TableHeaderName"]); 
				$data["field_name"] = $this->addslashesx(  $jsonArr["FieldName"]); 
				$data["num"] = $this->addslashesx(  $jsonArr["Num"]); 
				$data["tag"] = $this->addslashesx(  $jsonArr["Tag"]); 



				$su =  $tDownloadColumnModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or catalog_enum like '%$searchkey%' or table_header_name like '%$searchkey%' or field_name like '%$searchkey%' or num like '%$searchkey%' or tag like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tDownloadColumnModel->where($selectWhere )->count();
		...
		$listData = $tDownloadColumnModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."t_download_column" ;
...
;
		...
		$tDownloadColumnModel = D("TDownloadColumn");
    	$tmpArr = $tDownloadColumnModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "t_download_column".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"t_download_column",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["table_header_name"] = addslashes(  $_REQUEST["addData_table_header_name"] ); 
				$data["field_name"] = addslashes(  $_REQUEST["addData_field_name"] ); 
				$data["num"] = addslashes(  $_REQUEST["addData_num"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["table_header_name"] =  addslashes( $_REQUEST["editData_table_header_name"]); 
				$data["field_name"] =  addslashes( $_REQUEST["editData_field_name"]); 
				$data["num"] =  addslashes( $_REQUEST["editData_num"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tDownloadColumnModel = D("TDownloadColumn");
        $editData = $tDownloadColumnModel->getById($params);
...
;
        ...
        $this->redirect("TDownloadColumn/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$t_download_columnModel = D ( "TDownloadColumn" );
				$tmpArr = $t_download_columnModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$t_download_columnModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $t_download_columnModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TDownloadColumn'>返回</a>";
			}
		} else {
			print "upload error";
		}
