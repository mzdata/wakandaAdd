...
//$wxStoreAppConfigModel = D("WxStoreAppConfig"); //BASECIDE0
// |WxStoreAppConfig\/queryOneMyData$
// |WxStoreAppConfig\/delMyData$
// |WxStoreAppConfig\/addMyData$
// |WxStoreAppConfig\/updateMyData$
// |WxStoreAppConfig\/queryList$
/*
  id  id 
...
class WxStoreAppConfigAction extends  Action {
	...
	//WxStoreAppConfig\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$wxStoreAppConfigModel = D("WxStoreAppConfig");
				$editData = $wxStoreAppConfigModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$wxStoreAppConfigModel = D("WxStoreAppConfig");
					$sc = 	$wxStoreAppConfigModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["permissionId"]))		$data["permission_id"] =    $this->addslashesx($jsonArr["permissionId"]); 
		if(isset($jsonArr["display"]))		$data["display"] =    $this->addslashesx($jsonArr["display"]); 
		if(isset($jsonArr["type"]))		$data["type"] =    $this->addslashesx($jsonArr["type"]); 

            $su = $wxStoreAppConfigModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["permission_id"] = $this->addslashesx(  $jsonArr["PermissionId"]); 
				$data["display"] = $this->addslashesx(  $jsonArr["Display"]); 
				$data["type"] = $this->addslashesx(  $jsonArr["Type"]); 



				$su =  $wxStoreAppConfigModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or permission_id like '%$searchkey%' or display like '%$searchkey%' or type like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $wxStoreAppConfigModel->where($selectWhere )->count();
		...
		$listData = $wxStoreAppConfigModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."wx_store_app_config" ;
...
;
		...
		$wxStoreAppConfigModel = D("WxStoreAppConfig");
    	$tmpArr = $wxStoreAppConfigModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "wx_store_app_config".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"wx_store_app_config",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["permission_id"] = addslashes(  $_REQUEST["addData_permission_id"] ); 
				$data["display"] = addslashes(  $_REQUEST["addData_display"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["permission_id"] =  addslashes( $_REQUEST["editData_permission_id"]); 
				$data["display"] =  addslashes( $_REQUEST["editData_display"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$wxStoreAppConfigModel = D("WxStoreAppConfig");
        $editData = $wxStoreAppConfigModel->getById($params);
...
;
        ...
        $this->redirect("WxStoreAppConfig/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$wx_store_app_configModel = D ( "WxStoreAppConfig" );
				$tmpArr = $wx_store_app_configModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$wx_store_app_configModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $wx_store_app_configModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../WxStoreAppConfig'>返回</a>";
			}
		} else {
			print "upload error";
		}
