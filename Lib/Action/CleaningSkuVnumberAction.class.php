...
//$cleaningSkuVnumberModel = D("CleaningSkuVnumber"); //BASECIDE0
// |CleaningSkuVnumber\/queryOneMyData$
// |CleaningSkuVnumber\/delMyData$
// |CleaningSkuVnumber\/addMyData$
// |CleaningSkuVnumber\/updateMyData$
// |CleaningSkuVnumber\/queryList$
/**/

class CleaningSkuVnumberAction extends  Action {
	...
	//CleaningSkuVnumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$cleaningSkuVnumberModel = D("CleaningSkuVnumber");
				$editData = $cleaningSkuVnumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$cleaningSkuVnumberModel = D("CleaningSkuVnumber");
					$sc = 	$cleaningSkuVnumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $cleaningSkuVnumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $cleaningSkuVnumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $cleaningSkuVnumberModel->where($selectWhere )->count();
		...
		$listData = $cleaningSkuVnumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."cleaning_sku_vnumber" ;
...
;
		...
		$cleaningSkuVnumberModel = D("CleaningSkuVnumber");
    	$tmpArr = $cleaningSkuVnumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "cleaning_sku_vnumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"cleaning_sku_vnumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$cleaningSkuVnumberModel = D("CleaningSkuVnumber");
        $editData = $cleaningSkuVnumberModel->getById($params);
...
;
        ...
        $this->redirect("CleaningSkuVnumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$cleaning_sku_vnumberModel = D ( "CleaningSkuVnumber" );
				$tmpArr = $cleaning_sku_vnumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$cleaning_sku_vnumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $cleaning_sku_vnumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../CleaningSkuVnumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
