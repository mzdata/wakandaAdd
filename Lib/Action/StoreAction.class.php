...
//$storeModel = D("Store"); //BASECIDE0
// |Store\/queryOneMyData$
// |Store\/delMyData$
// |Store\/addMyData$
// |Store\/updateMyData$
// |Store\/queryList$
...
class StoreAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$storeModel = D("Store");
				$editData = $storeModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$storeModel = D("Store");
					$sc = 	$storeModel->where("id='$id'")->delete();
    ...
    public function queryList() {
        ...
        $count = $storeModel->where($selectWhere )->count();
		...
		$listData = $storeModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"store",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$storeModel = D("Store");
        $editData = $storeModel->getById($params);
