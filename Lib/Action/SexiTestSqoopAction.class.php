...
//$sexiTestSqoopModel = D("SexiTestSqoop"); //BASECIDE0
// |SexiTestSqoop\/queryOneMyData$
// |SexiTestSqoop\/delMyData$
// |SexiTestSqoop\/addMyData$
// |SexiTestSqoop\/updateMyData$
// |SexiTestSqoop\/queryList$
/*
  id  id 
  tenantCode  租户code 
  taskId  task_id 
...
class SexiTestSqoopAction extends  Action {
	...
	//SexiTestSqoop\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sexiTestSqoopModel = D("SexiTestSqoop");
				$editData = $sexiTestSqoopModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sexiTestSqoopModel = D("SexiTestSqoop");
					$sc = 	$sexiTestSqoopModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["dt"]))		$data["dt"] =    $this->addslashesx($jsonArr["dt"]); 
            ...
            $su = $sexiTestSqoopModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $sexiTestSqoopModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or task_id like '%$searchkey%' or period like '%$searchkey%' or materia like '%$searchkey%' or tenant_type like '%$searchkey%' or warehouse_name like '%$searchkey%' or sub_company like '%$searchkey%' or agent like '%$searchkey%' or store_id like '%$searchkey%' or store_name like '%$searchkey%' or province like '%$searchkey%' or city like '%$searchkey%' or county like '%$searchkey%' or yl like '%$searchkey%' or jh_cnt like '%$searchkey%' or is_alert like '%$searchkey%' or alert_reason like '%$searchkey%' or dt like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $sexiTestSqoopModel->where($selectWhere )->count();
		...
		$listData = $sexiTestSqoopModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."sexi_test_sqoop" ;
...
;
		...
		$sexiTestSqoopModel = D("SexiTestSqoop");
    	$tmpArr = $sexiTestSqoopModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sexi_test_sqoop".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sexi_test_sqoop",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$sexiTestSqoopModel = D("SexiTestSqoop");
        $editData = $sexiTestSqoopModel->getById($params);
...
;
        ...
        $this->redirect("SexiTestSqoop/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sexi_test_sqoopModel = D ( "SexiTestSqoop" );
				$tmpArr = $sexi_test_sqoopModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sexi_test_sqoopModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sexi_test_sqoopModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../SexiTestSqoop'>返回</a>";
			}
		} else {
			print "upload error";
		}
