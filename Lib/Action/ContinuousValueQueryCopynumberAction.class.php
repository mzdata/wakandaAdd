...
//$continuousValueQueryCopynumberModel = D("ContinuousValueQueryCopynumber"); //BASECIDE0
// |ContinuousValueQueryCopynumber\/queryOneMyData$
// |ContinuousValueQueryCopynumber\/delMyData$
// |ContinuousValueQueryCopynumber\/addMyData$
// |ContinuousValueQueryCopynumber\/updateMyData$
// |ContinuousValueQueryCopynumber\/queryList$
/**/

class ContinuousValueQueryCopynumberAction extends  Action {
	...
	//ContinuousValueQueryCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$continuousValueQueryCopynumberModel = D("ContinuousValueQueryCopynumber");
				$editData = $continuousValueQueryCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$continuousValueQueryCopynumberModel = D("ContinuousValueQueryCopynumber");
					$sc = 	$continuousValueQueryCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $continuousValueQueryCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $continuousValueQueryCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $continuousValueQueryCopynumberModel->where($selectWhere )->count();
		...
		$listData = $continuousValueQueryCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."continuous_value_query_copynumber" ;
...
;
		...
		$continuousValueQueryCopynumberModel = D("ContinuousValueQueryCopynumber");
    	$tmpArr = $continuousValueQueryCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "continuous_value_query_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"continuous_value_query_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$continuousValueQueryCopynumberModel = D("ContinuousValueQueryCopynumber");
        $editData = $continuousValueQueryCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("ContinuousValueQueryCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$continuous_value_query_copynumberModel = D ( "ContinuousValueQueryCopynumber" );
				$tmpArr = $continuous_value_query_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$continuous_value_query_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $continuous_value_query_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../ContinuousValueQueryCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
