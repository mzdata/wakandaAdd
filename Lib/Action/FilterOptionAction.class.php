...
//$filterOptionModel = D("FilterOption"); //BASECIDE0
// |FilterOption\/queryOneMyData$
// |FilterOption\/delMyData$
// |FilterOption\/addMyData$
// |FilterOption\/updateMyData$
// |FilterOption\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  tenantName  租户名称 
...
class FilterOptionAction extends  Action {
	...
	//FilterOption\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterOptionModel = D("FilterOption");
				$editData = $filterOptionModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterOptionModel = D("FilterOption");
					$sc = 	$filterOptionModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["deleted"]))		$data["deleted"] =    $this->addslashesx($jsonArr["deleted"]); 
            ...
            $su = $filterOptionModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $filterOptionModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $filterOptionModel->where($selectWhere )->count();
		...
		$listData = $filterOptionModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."filter_option" ;
...
;
		...
		$filterOptionModel = D("FilterOption");
    	$tmpArr = $filterOptionModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "filter_option".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"filter_option",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$filterOptionModel = D("FilterOption");
        $editData = $filterOptionModel->getById($params);
...
;
        ...
        $this->redirect("FilterOption/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$filter_optionModel = D ( "FilterOption" );
				$tmpArr = $filter_optionModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$filter_optionModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $filter_optionModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../FilterOption'>返回</a>";
			}
		} else {
			print "upload error";
		}
