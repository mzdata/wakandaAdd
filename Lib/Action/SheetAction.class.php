...
//$sheetModel = D("Sheet"); //BASECIDE0
// |Sheet\/queryOneMyData$
// |Sheet\/delMyData$
// |Sheet\/addMyData$
// |Sheet\/updateMyData$
// |Sheet\/queryList$
...
class SheetAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sheetModel = D("Sheet");
				$editData = $sheetModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sheetModel = D("Sheet");
					$sc = 	$sheetModel->where("id='$id'")->delete();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or value like '%$searchkey%' or name like '%$searchkey%' or type like '%$searchkey%' or tenant_code like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $sheetModel->where($selectWhere )->count();
		...
		$listData = $sheetModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sheet",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$sheetModel = D("Sheet");
        $editData = $sheetModel->getById($params);
