...
//$tbDataassetsRemaindownloadModel = D("TbDataassetsRemaindownload"); //BASECIDE0
// |TbDataassetsRemaindownload\/queryOneMyData$
// |TbDataassetsRemaindownload\/delMyData$
// |TbDataassetsRemaindownload\/addMyData$
// |TbDataassetsRemaindownload\/updateMyData$
// |TbDataassetsRemaindownload\/queryList$
/*
  id  id 
  tenantCode  租户code 
  remainDownloads  剩余下载次数 */

class TbDataassetsRemaindownloadAction extends  Action {
	...
	//TbDataassetsRemaindownload\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbDataassetsRemaindownloadModel = D("TbDataassetsRemaindownload");
				$editData = $tbDataassetsRemaindownloadModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbDataassetsRemaindownloadModel = D("TbDataassetsRemaindownload");
					$sc = 	$tbDataassetsRemaindownloadModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["remainDownloads"]))		$data["remain_downloads"] =    $this->addslashesx($jsonArr["remainDownloads"]); 

            $su = $tbDataassetsRemaindownloadModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["remain_downloads"] = $this->addslashesx(  $jsonArr["RemainDownloads"]); 



				$su =  $tbDataassetsRemaindownloadModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or remain_downloads like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tbDataassetsRemaindownloadModel->where($selectWhere )->count();
		...
		$listData = $tbDataassetsRemaindownloadModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_dataassets_remaindownload" ;
...
;
		...
		$tbDataassetsRemaindownloadModel = D("TbDataassetsRemaindownload");
    	$tmpArr = $tbDataassetsRemaindownloadModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_dataassets_remaindownload".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_dataassets_remaindownload",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["remain_downloads"] = addslashes(  $_REQUEST["addData_remain_downloads"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["remain_downloads"] =  addslashes( $_REQUEST["editData_remain_downloads"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbDataassetsRemaindownloadModel = D("TbDataassetsRemaindownload");
        $editData = $tbDataassetsRemaindownloadModel->getById($params);
...
;
        ...
        $this->redirect("TbDataassetsRemaindownload/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_dataassets_remaindownloadModel = D ( "TbDataassetsRemaindownload" );
				$tmpArr = $tb_dataassets_remaindownloadModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_dataassets_remaindownloadModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_dataassets_remaindownloadModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TbDataassetsRemaindownload'>返回</a>";
			}
		} else {
			print "upload error";
		}
