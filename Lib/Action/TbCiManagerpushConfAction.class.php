...
//$tbCiManagerpushConfModel = D("TbCiManagerpushConf"); //BASECIDE0
// |TbCiManagerpushConf\/queryOneMyData$
// |TbCiManagerpushConf\/delMyData$
// |TbCiManagerpushConf\/addMyData$
// |TbCiManagerpushConf\/updateMyData$
// |TbCiManagerpushConf\/queryList$
/*
  id  id 
  tenantCode  租户code 
  managerpushConf  配置 */

class TbCiManagerpushConfAction extends  Action {
	...
	//TbCiManagerpushConf\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbCiManagerpushConfModel = D("TbCiManagerpushConf");
				$editData = $tbCiManagerpushConfModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbCiManagerpushConfModel = D("TbCiManagerpushConf");
					$sc = 	$tbCiManagerpushConfModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["managerpushConf"]))		$data["managerpush_conf"] =    $this->addslashesx($jsonArr["managerpushConf"]); 

            $su = $tbCiManagerpushConfModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["managerpush_conf"] = $this->addslashesx(  $jsonArr["ManagerpushConf"]); 



				$su =  $tbCiManagerpushConfModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or managerpush_conf like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tbCiManagerpushConfModel->where($selectWhere )->count();
		...
		$listData = $tbCiManagerpushConfModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_ci_managerpush_conf" ;
...
;
		...
		$tbCiManagerpushConfModel = D("TbCiManagerpushConf");
    	$tmpArr = $tbCiManagerpushConfModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_ci_managerpush_conf".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_ci_managerpush_conf",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["managerpush_conf"] = addslashes(  $_REQUEST["addData_managerpush_conf"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["managerpush_conf"] =  addslashes( $_REQUEST["editData_managerpush_conf"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbCiManagerpushConfModel = D("TbCiManagerpushConf");
        $editData = $tbCiManagerpushConfModel->getById($params);
...
;
        ...
        $this->redirect("TbCiManagerpushConf/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_ci_managerpush_confModel = D ( "TbCiManagerpushConf" );
				$tmpArr = $tb_ci_managerpush_confModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_ci_managerpush_confModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_ci_managerpush_confModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TbCiManagerpushConf'>返回</a>";
			}
		} else {
			print "upload error";
		}
