...
//$adsPurchaseAlertStoreDayPreviewModel = D("AdsPurchaseAlertStoreDayPreview"); //BASECIDE0
// |AdsPurchaseAlertStoreDayPreview\/queryOneMyData$
// |AdsPurchaseAlertStoreDayPreview\/delMyData$
// |AdsPurchaseAlertStoreDayPreview\/addMyData$
// |AdsPurchaseAlertStoreDayPreview\/updateMyData$
// |AdsPurchaseAlertStoreDayPreview\/queryList$
/*
  id  id 
  tenantCode  租户code 
  taskId  task_id 
...
class AdsPurchaseAlertStoreDayPreviewAction extends  Action {
	...
	//AdsPurchaseAlertStoreDayPreview\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$adsPurchaseAlertStoreDayPreviewModel = D("AdsPurchaseAlertStoreDayPreview");
				$editData = $adsPurchaseAlertStoreDayPreviewModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$adsPurchaseAlertStoreDayPreviewModel = D("AdsPurchaseAlertStoreDayPreview");
					$sc = 	$adsPurchaseAlertStoreDayPreviewModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["dt"]))		$data["dt"] =    $this->addslashesx($jsonArr["dt"]); 
            ...
            $su = $adsPurchaseAlertStoreDayPreviewModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $adsPurchaseAlertStoreDayPreviewModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $adsPurchaseAlertStoreDayPreviewModel->where($selectWhere )->count();
		...
		$listData = $adsPurchaseAlertStoreDayPreviewModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."ads_purchase_alert_store_day_preview" ;
...
;
		...
		$adsPurchaseAlertStoreDayPreviewModel = D("AdsPurchaseAlertStoreDayPreview");
    	$tmpArr = $adsPurchaseAlertStoreDayPreviewModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "ads_purchase_alert_store_day_preview".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"ads_purchase_alert_store_day_preview",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$adsPurchaseAlertStoreDayPreviewModel = D("AdsPurchaseAlertStoreDayPreview");
        $editData = $adsPurchaseAlertStoreDayPreviewModel->getById($params);
...
;
        ...
        $this->redirect("AdsPurchaseAlertStoreDayPreview/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$ads_purchase_alert_store_day_previewModel = D ( "AdsPurchaseAlertStoreDayPreview" );
				$tmpArr = $ads_purchase_alert_store_day_previewModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$ads_purchase_alert_store_day_previewModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $ads_purchase_alert_store_day_previewModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../AdsPurchaseAlertStoreDayPreview'>返回</a>";
			}
		} else {
			print "upload error";
		}
