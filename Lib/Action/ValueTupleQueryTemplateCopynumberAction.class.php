...
//$valueTupleQueryTemplateCopynumberModel = D("ValueTupleQueryTemplateCopynumber"); //BASECIDE0
// |ValueTupleQueryTemplateCopynumber\/queryOneMyData$
// |ValueTupleQueryTemplateCopynumber\/delMyData$
// |ValueTupleQueryTemplateCopynumber\/addMyData$
// |ValueTupleQueryTemplateCopynumber\/updateMyData$
// |ValueTupleQueryTemplateCopynumber\/queryList$
/**/

class ValueTupleQueryTemplateCopynumberAction extends  Action {
	...
	//ValueTupleQueryTemplateCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$valueTupleQueryTemplateCopynumberModel = D("ValueTupleQueryTemplateCopynumber");
				$editData = $valueTupleQueryTemplateCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$valueTupleQueryTemplateCopynumberModel = D("ValueTupleQueryTemplateCopynumber");
					$sc = 	$valueTupleQueryTemplateCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $valueTupleQueryTemplateCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $valueTupleQueryTemplateCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $valueTupleQueryTemplateCopynumberModel->where($selectWhere )->count();
		...
		$listData = $valueTupleQueryTemplateCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."value_tuple_query_template_copynumber" ;
...
;
		...
		$valueTupleQueryTemplateCopynumberModel = D("ValueTupleQueryTemplateCopynumber");
    	$tmpArr = $valueTupleQueryTemplateCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "value_tuple_query_template_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"value_tuple_query_template_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$valueTupleQueryTemplateCopynumberModel = D("ValueTupleQueryTemplateCopynumber");
        $editData = $valueTupleQueryTemplateCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("ValueTupleQueryTemplateCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$value_tuple_query_template_copynumberModel = D ( "ValueTupleQueryTemplateCopynumber" );
				$tmpArr = $value_tuple_query_template_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$value_tuple_query_template_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $value_tuple_query_template_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../ValueTupleQueryTemplateCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
