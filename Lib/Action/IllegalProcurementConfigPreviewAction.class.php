...
//$illegalProcurementConfigPreviewModel = D("IllegalProcurementConfigPreview"); //BASECIDE0
// |IllegalProcurementConfigPreview\/queryOneMyData$
// |IllegalProcurementConfigPreview\/delMyData$
// |IllegalProcurementConfigPreview\/addMyData$
// |IllegalProcurementConfigPreview\/updateMyData$
// |IllegalProcurementConfigPreview\/queryList$
/*
  id  id 
  tenantCode  租户 
  period  采样周期 
...
class IllegalProcurementConfigPreviewAction extends  Action {
	...
	//IllegalProcurementConfigPreview\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$illegalProcurementConfigPreviewModel = D("IllegalProcurementConfigPreview");
				$editData = $illegalProcurementConfigPreviewModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$illegalProcurementConfigPreviewModel = D("IllegalProcurementConfigPreview");
					$sc = 	$illegalProcurementConfigPreviewModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["conditions"]))		$data["conditions"] =    $this->addslashesx($jsonArr["conditions"]); 
		if(isset($jsonArr["querySql"]))		$data["query_sql"] =    $this->addslashesx($jsonArr["querySql"]); 
		...
		if(isset($jsonArr["updateTime"]))		$data["update_time"] =    $this->addslashesx($jsonArr["updateTime"]); 
            ...
            $su = $illegalProcurementConfigPreviewModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["conditions"] = $this->addslashesx(  $jsonArr["Conditions"]); 
				$data["query_sql"] = $this->addslashesx(  $jsonArr["QuerySql"]); 
				$data["create_time"] = $this->addslashesx(  $jsonArr["CreateTime"]); 
				$data["update_time"] = $this->addslashesx(  $jsonArr["UpdateTime"]); 



				$su =  $illegalProcurementConfigPreviewModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or period like '%$searchkey%' or conditions like '%$searchkey%' or query_sql like '%$searchkey%' or create_time like '%$searchkey%' or update_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $illegalProcurementConfigPreviewModel->where($selectWhere )->count();
		...
		$listData = $illegalProcurementConfigPreviewModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."illegal_procurement_config_preview" ;
...
;
		...
		$illegalProcurementConfigPreviewModel = D("IllegalProcurementConfigPreview");
    	$tmpArr = $illegalProcurementConfigPreviewModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "illegal_procurement_config_preview".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"illegal_procurement_config_preview",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["conditions"] = addslashes(  $_REQUEST["addData_conditions"] ); 
				$data["query_sql"] = addslashes(  $_REQUEST["addData_query_sql"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["conditions"] =  addslashes( $_REQUEST["editData_conditions"]); 
				$data["query_sql"] =  addslashes( $_REQUEST["editData_query_sql"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$illegalProcurementConfigPreviewModel = D("IllegalProcurementConfigPreview");
        $editData = $illegalProcurementConfigPreviewModel->getById($params);
...
;
        ...
        $this->redirect("IllegalProcurementConfigPreview/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$illegal_procurement_config_previewModel = D ( "IllegalProcurementConfigPreview" );
				$tmpArr = $illegal_procurement_config_previewModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$illegal_procurement_config_previewModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $illegal_procurement_config_previewModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../IllegalProcurementConfigPreview'>返回</a>";
			}
		} else {
			print "upload error";
		}
