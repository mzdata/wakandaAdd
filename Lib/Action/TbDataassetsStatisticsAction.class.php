...
//$tbDataassetsStatisticsModel = D("TbDataassetsStatistics"); //BASECIDE0
// |TbDataassetsStatistics\/queryOneMyData$
// |TbDataassetsStatistics\/delMyData$
// |TbDataassetsStatistics\/addMyData$
// |TbDataassetsStatistics\/updateMyData$
// |TbDataassetsStatistics\/queryList$
/*
  id  id 
  tenantCode  租户code 
  bizTag  member、trade、market、supply_china 
  tableName  表名 
  tableType  导入方式：全量或增量 
  dateStr  日期 
  yearStr  年的字符串 
  storage  unsigned 
  number  unsigned 
...
class TbDataassetsStatisticsAction extends  Action {
	...
	//TbDataassetsStatistics\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
				...
				$editData = $tbDataassetsStatisticsModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbDataassetsStatisticsModel = D("TbDataassetsStatistics");
					$sc = 	$tbDataassetsStatisticsModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["dateStr"]))		$data["date_str"] =    $this->addslashesx($jsonArr["dateStr"]); 
		if(isset($jsonArr["yearStr"]))		$data["year_str"] =    $this->addslashesx($jsonArr["yearStr"]); 
		if(isset($jsonArr["storage"]))		$data["storage"] =    $this->addslashesx($jsonArr["storage"]); 
		if(isset($jsonArr["number"]))		$data["number"] =    $this->addslashesx($jsonArr["number"]); 
		if(isset($jsonArr["updateTime"]))		$data["update_time"] =    $this->addslashesx($jsonArr["updateTime"]); 

            $su = $tbDataassetsStatisticsModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["date_str"] = $this->addslashesx(  $jsonArr["DateStr"]); 
				$data["year_str"] = $this->addslashesx(  $jsonArr["YearStr"]); 
				$data["storage"] = $this->addslashesx(  $jsonArr["Storage"]); 
				$data["number"] = $this->addslashesx(  $jsonArr["Number"]); 
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or biz_tag like '%$searchkey%' or table_name like '%$searchkey%' or table_type like '%$searchkey%' or date_str like '%$searchkey%' or year_str like '%$searchkey%' or storage like '%$searchkey%' or number like '%$searchkey%' or update_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tbDataassetsStatisticsModel->where($selectWhere )->count();
		...
		$listData = $tbDataassetsStatisticsModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_dataassets_statistics" ;
...
;
		...
		$tbDataassetsStatisticsModel = D("TbDataassetsStatistics");
    	$tmpArr = $tbDataassetsStatisticsModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_dataassets_statistics".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_dataassets_statistics",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["date_str"] = addslashes(  $_REQUEST["addData_date_str"] ); 
				$data["year_str"] = addslashes(  $_REQUEST["addData_year_str"] ); 
				$data["storage"] = addslashes(  $_REQUEST["addData_storage"] ); 
				$data["number"] = addslashes(  $_REQUEST["addData_number"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["date_str"] =  addslashes( $_REQUEST["editData_date_str"]); 
				$data["year_str"] =  addslashes( $_REQUEST["editData_year_str"]); 
				$data["storage"] =  addslashes( $_REQUEST["editData_storage"]); 
				$data["number"] =  addslashes( $_REQUEST["editData_number"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbDataassetsStatisticsModel = D("TbDataassetsStatistics");
        $editData = $tbDataassetsStatisticsModel->getById($params);
...
;
        ...
        $this->redirect("TbDataassetsStatistics/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_dataassets_statisticsModel = D ( "TbDataassetsStatistics" );
				$tmpArr = $tb_dataassets_statisticsModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_dataassets_statisticsModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_dataassets_statisticsModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TbDataassetsStatistics'>返回</a>";
			}
		} else {
			print "upload error";
		}
