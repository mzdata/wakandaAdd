...
class IdsAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$idsModel = D("Ids");
					$sc = 	$idsModel->where("id='$id'")->delete();
    ...
    public function queryList() {
        ...
        $count = $idsModel->where($selectWhere )->count();
		...
		$listData = $idsModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"ids",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$idsModel = D("Ids");
        $editData = $idsModel->getById($params);
