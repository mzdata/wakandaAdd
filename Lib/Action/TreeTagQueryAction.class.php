...
//$treeTagQueryModel = D("TreeTagQuery"); //BASECIDE0
// |TreeTagQuery\/queryOneMyData$
// |TreeTagQuery\/delMyData$
// |TreeTagQuery\/addMyData$
// |TreeTagQuery\/updateMyData$
// |TreeTagQuery\/queryList$
/*
  id  id 
...
class TreeTagQueryAction extends  Action {
	...
	//TreeTagQuery\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeTagQueryModel = D("TreeTagQuery");
				$editData = $treeTagQueryModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeTagQueryModel = D("TreeTagQuery");
					$sc = 	$treeTagQueryModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["sql"]))		$data["sql"] =    $this->addslashesx($jsonArr["sql"]); 

            $su = $treeTagQueryModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["sql"] = $this->addslashesx(  $jsonArr["Sql"]); 



				$su =  $treeTagQueryModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or tag_id like '%$searchkey%' or tag like '%$searchkey%' or type like '%$searchkey%' or deleted like '%$searchkey%' or sql like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $treeTagQueryModel->where($selectWhere )->count();
		...
		$listData = $treeTagQueryModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tree_tag_query" ;
...
;
		...
		$treeTagQueryModel = D("TreeTagQuery");
    	$tmpArr = $treeTagQueryModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_tag_query".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_tag_query",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["sql"] = addslashes(  $_REQUEST["addData_sql"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["sql"] =  addslashes( $_REQUEST["editData_sql"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeTagQueryModel = D("TreeTagQuery");
        $editData = $treeTagQueryModel->getById($params);
...
;
        ...
        $this->redirect("TreeTagQuery/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_tag_queryModel = D ( "TreeTagQuery" );
				$tmpArr = $tree_tag_queryModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_tag_queryModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_tag_queryModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TreeTagQuery'>返回</a>";
			}
		} else {
			print "upload error";
		}
