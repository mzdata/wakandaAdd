...
//$dimCommodityListDfModel = D("DimCommodityListDf"); //BASECIDE0
// |DimCommodityListDf\/queryOneMyData$
// |DimCommodityListDf\/delMyData$
// |DimCommodityListDf\/addMyData$
// |DimCommodityListDf\/updateMyData$
// |DimCommodityListDf\/queryList$
/*
  id  id 
  tenantCode  租户code 
  commodityId  商品id 
...
class DimCommodityListDfAction extends  Action {
	...
	//DimCommodityListDf\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$dimCommodityListDfModel = D("DimCommodityListDf");
				$editData = $dimCommodityListDfModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$dimCommodityListDfModel = D("DimCommodityListDf");
					$sc = 	$dimCommodityListDfModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updateTime"]))		$data["update_time"] =    $this->addslashesx($jsonArr["updateTime"]); 
            ...
            $su = $dimCommodityListDfModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $dimCommodityListDfModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or commodity_id like '%$searchkey%' or commodity_name like '%$searchkey%' or category_id like '%$searchkey%' or category_name like '%$searchkey%' or create_time like '%$searchkey%' or update_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $dimCommodityListDfModel->where($selectWhere )->count();
		...
		$listData = $dimCommodityListDfModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."dim_commodity_list_df" ;
...
;
		...
		$dimCommodityListDfModel = D("DimCommodityListDf");
    	$tmpArr = $dimCommodityListDfModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "dim_commodity_list_df".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"dim_commodity_list_df",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$dimCommodityListDfModel = D("DimCommodityListDf");
        $editData = $dimCommodityListDfModel->getById($params);
...
;
        ...
        $this->redirect("DimCommodityListDf/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$dim_commodity_list_dfModel = D ( "DimCommodityListDf" );
				$tmpArr = $dim_commodity_list_dfModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$dim_commodity_list_dfModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $dim_commodity_list_dfModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../DimCommodityListDf'>返回</a>";
			}
		} else {
			print "upload error";
		}
