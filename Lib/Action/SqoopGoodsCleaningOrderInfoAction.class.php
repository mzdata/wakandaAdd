...
//$sqoopGoodsCleaningOrderInfoModel = D("SqoopGoodsCleaningOrderInfo"); //BASECIDE0
// |SqoopGoodsCleaningOrderInfo\/queryOneMyData$
// |SqoopGoodsCleaningOrderInfo\/delMyData$
// |SqoopGoodsCleaningOrderInfo\/addMyData$
// |SqoopGoodsCleaningOrderInfo\/updateMyData$
// |SqoopGoodsCleaningOrderInfo\/queryList$
/*
  id  id 
  time  开始日期 
  cnt  清洗数量 
...
class SqoopGoodsCleaningOrderInfoAction extends  Action {
	...
	//SqoopGoodsCleaningOrderInfo\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqoopGoodsCleaningOrderInfoModel = D("SqoopGoodsCleaningOrderInfo");
				$editData = $sqoopGoodsCleaningOrderInfoModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqoopGoodsCleaningOrderInfoModel = D("SqoopGoodsCleaningOrderInfo");
					$sc = 	$sqoopGoodsCleaningOrderInfoModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["time"]))		$data["time"] =    $this->addslashesx($jsonArr["time"]); 
		...
		if(isset($jsonArr["tenantCode"]))		$data["tenant_code"] =    $this->addslashesx($jsonArr["tenantCode"]); 
            ...
            $su = $sqoopGoodsCleaningOrderInfoModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
 				...
 				$data = array();
									...
									$data["time"] = $this->addslashesx(  $jsonArr["Time"]); 
				...
				$su =  $sqoopGoodsCleaningOrderInfoModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or time like '%$searchkey%' or cnt like '%$searchkey%' or updated_time like '%$searchkey%' or tenant_code like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $sqoopGoodsCleaningOrderInfoModel->where($selectWhere )->count();
		...
		$listData = $sqoopGoodsCleaningOrderInfoModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."sqoop_goods_cleaning_order_info" ;
...
;
		...
		$sqoopGoodsCleaningOrderInfoModel = D("SqoopGoodsCleaningOrderInfo");
    	$tmpArr = $sqoopGoodsCleaningOrderInfoModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sqoop_goods_cleaning_order_info".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sqoop_goods_cleaning_order_info",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["time"] = addslashes(  $_REQUEST["addData_time"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["time"] =  addslashes( $_REQUEST["editData_time"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$sqoopGoodsCleaningOrderInfoModel = D("SqoopGoodsCleaningOrderInfo");
        $editData = $sqoopGoodsCleaningOrderInfoModel->getById($params);
...
;
        ...
        $this->redirect("SqoopGoodsCleaningOrderInfo/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sqoop_goods_cleaning_order_infoModel = D ( "SqoopGoodsCleaningOrderInfo" );
				$tmpArr = $sqoop_goods_cleaning_order_infoModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sqoop_goods_cleaning_order_infoModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sqoop_goods_cleaning_order_infoModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../SqoopGoodsCleaningOrderInfo'>返回</a>";
			}
		} else {
			print "upload error";
		}
