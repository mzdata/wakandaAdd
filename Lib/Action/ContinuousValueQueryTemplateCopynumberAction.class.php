...
//$continuousValueQueryTemplateCopynumberModel = D("ContinuousValueQueryTemplateCopynumber"); //BASECIDE0
// |ContinuousValueQueryTemplateCopynumber\/queryOneMyData$
// |ContinuousValueQueryTemplateCopynumber\/delMyData$
// |ContinuousValueQueryTemplateCopynumber\/addMyData$
// |ContinuousValueQueryTemplateCopynumber\/updateMyData$
// |ContinuousValueQueryTemplateCopynumber\/queryList$
/**/

class ContinuousValueQueryTemplateCopynumberAction extends  Action {
	...
	//ContinuousValueQueryTemplateCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$continuousValueQueryTemplateCopynumberModel = D("ContinuousValueQueryTemplateCopynumber");
				$editData = $continuousValueQueryTemplateCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$continuousValueQueryTemplateCopynumberModel = D("ContinuousValueQueryTemplateCopynumber");
					$sc = 	$continuousValueQueryTemplateCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $continuousValueQueryTemplateCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $continuousValueQueryTemplateCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $continuousValueQueryTemplateCopynumberModel->where($selectWhere )->count();
		...
		$listData = $continuousValueQueryTemplateCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."continuous_value_query_template_copynumber" ;
...
;
		...
		$continuousValueQueryTemplateCopynumberModel = D("ContinuousValueQueryTemplateCopynumber");
    	$tmpArr = $continuousValueQueryTemplateCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "continuous_value_query_template_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"continuous_value_query_template_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$continuousValueQueryTemplateCopynumberModel = D("ContinuousValueQueryTemplateCopynumber");
        $editData = $continuousValueQueryTemplateCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("ContinuousValueQueryTemplateCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$continuous_value_query_template_copynumberModel = D ( "ContinuousValueQueryTemplateCopynumber" );
				$tmpArr = $continuous_value_query_template_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$continuous_value_query_template_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $continuous_value_query_template_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../ContinuousValueQueryTemplateCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
