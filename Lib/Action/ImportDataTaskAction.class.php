...
//$importDataTaskModel = D("ImportDataTask"); //BASECIDE0
// |ImportDataTask\/queryOneMyData$
// |ImportDataTask\/delMyData$
// |ImportDataTask\/addMyData$
// |ImportDataTask\/updateMyData$
// |ImportDataTask\/queryList$
/*
  id  id 
...
class ImportDataTaskAction extends  Action {
	...
	//ImportDataTask\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$importDataTaskModel = D("ImportDataTask");
				$editData = $importDataTaskModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$importDataTaskModel = D("ImportDataTask");
					$sc = 	$importDataTaskModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["taskTime"]))		$data["task_time"] =    $this->addslashesx($jsonArr["taskTime"]); 
		if(isset($jsonArr["taskStatus"]))		$data["task_status"] =    $this->addslashesx($jsonArr["taskStatus"]); 

            $su = $importDataTaskModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["task_time"] = $this->addslashesx(  $jsonArr["TaskTime"]); 
				$data["task_status"] = $this->addslashesx(  $jsonArr["TaskStatus"]); 



				$su =  $importDataTaskModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or task_name like '%$searchkey%' or task_time like '%$searchkey%' or task_status like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $importDataTaskModel->where($selectWhere )->count();
		...
		$listData = $importDataTaskModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."import_data_task" ;
...
;
		...
		$importDataTaskModel = D("ImportDataTask");
    	$tmpArr = $importDataTaskModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "import_data_task".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"import_data_task",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["task_time"] = addslashes(  $_REQUEST["addData_task_time"] ); 
				$data["task_status"] = addslashes(  $_REQUEST["addData_task_status"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["task_time"] =  addslashes( $_REQUEST["editData_task_time"]); 
				$data["task_status"] =  addslashes( $_REQUEST["editData_task_status"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$importDataTaskModel = D("ImportDataTask");
        $editData = $importDataTaskModel->getById($params);
...
;
        ...
        $this->redirect("ImportDataTask/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$import_data_taskModel = D ( "ImportDataTask" );
				$tmpArr = $import_data_taskModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$import_data_taskModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $import_data_taskModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../ImportDataTask'>返回</a>";
			}
		} else {
			print "upload error";
		}
