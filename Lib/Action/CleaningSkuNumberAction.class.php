...
//$cleaningSkuNumberModel = D("CleaningSkuNumber"); //BASECIDE0
// |CleaningSkuNumber\/queryOneMyData$
// |CleaningSkuNumber\/delMyData$
// |CleaningSkuNumber\/addMyData$
// |CleaningSkuNumber\/updateMyData$
// |CleaningSkuNumber\/queryList$
/**/

class CleaningSkuNumberAction extends  Action {
	...
	//CleaningSkuNumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$cleaningSkuNumberModel = D("CleaningSkuNumber");
				$editData = $cleaningSkuNumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$cleaningSkuNumberModel = D("CleaningSkuNumber");
					$sc = 	$cleaningSkuNumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $cleaningSkuNumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $cleaningSkuNumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $cleaningSkuNumberModel->where($selectWhere )->count();
		...
		$listData = $cleaningSkuNumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."cleaning_sku_number" ;
...
;
		...
		$cleaningSkuNumberModel = D("CleaningSkuNumber");
    	$tmpArr = $cleaningSkuNumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "cleaning_sku_number".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"cleaning_sku_number",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$cleaningSkuNumberModel = D("CleaningSkuNumber");
        $editData = $cleaningSkuNumberModel->getById($params);
...
;
        ...
        $this->redirect("CleaningSkuNumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$cleaning_sku_numberModel = D ( "CleaningSkuNumber" );
				$tmpArr = $cleaning_sku_numberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$cleaning_sku_numberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $cleaning_sku_numberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../CleaningSkuNumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
