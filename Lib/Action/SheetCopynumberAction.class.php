...
//$sheetCopynumberModel = D("SheetCopynumber"); //BASECIDE0
// |SheetCopynumber\/queryOneMyData$
// |SheetCopynumber\/delMyData$
// |SheetCopynumber\/addMyData$
// |SheetCopynumber\/updateMyData$
// |SheetCopynumber\/queryList$
/**/

class SheetCopynumberAction extends  Action {
	...
	//SheetCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sheetCopynumberModel = D("SheetCopynumber");
				$editData = $sheetCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sheetCopynumberModel = D("SheetCopynumber");
					$sc = 	$sheetCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $sheetCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $sheetCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $sheetCopynumberModel->where($selectWhere )->count();
		...
		$listData = $sheetCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."sheet_copynumber" ;
...
;
		...
		$sheetCopynumberModel = D("SheetCopynumber");
    	$tmpArr = $sheetCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sheet_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sheet_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$sheetCopynumberModel = D("SheetCopynumber");
        $editData = $sheetCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("SheetCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sheet_copynumberModel = D ( "SheetCopynumber" );
				$tmpArr = $sheet_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sheet_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sheet_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../SheetCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
