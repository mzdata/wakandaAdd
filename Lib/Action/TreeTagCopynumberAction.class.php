...
//$treeTagCopynumberModel = D("TreeTagCopynumber"); //BASECIDE0
// |TreeTagCopynumber\/queryOneMyData$
// |TreeTagCopynumber\/delMyData$
// |TreeTagCopynumber\/addMyData$
// |TreeTagCopynumber\/updateMyData$
// |TreeTagCopynumber\/queryList$
/**/

class TreeTagCopynumberAction extends  Action {
	...
	//TreeTagCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeTagCopynumberModel = D("TreeTagCopynumber");
				$editData = $treeTagCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeTagCopynumberModel = D("TreeTagCopynumber");
					$sc = 	$treeTagCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $treeTagCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $treeTagCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $treeTagCopynumberModel->where($selectWhere )->count();
		...
		$listData = $treeTagCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tree_tag_copynumber" ;
...
;
		...
		$treeTagCopynumberModel = D("TreeTagCopynumber");
    	$tmpArr = $treeTagCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_tag_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_tag_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeTagCopynumberModel = D("TreeTagCopynumber");
        $editData = $treeTagCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("TreeTagCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_tag_copynumberModel = D ( "TreeTagCopynumber" );
				$tmpArr = $tree_tag_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_tag_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_tag_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TreeTagCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
