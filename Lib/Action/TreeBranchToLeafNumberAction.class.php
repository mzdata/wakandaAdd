...
//$treeBranchToLeafNumberModel = D("TreeBranchToLeafNumber"); //BASECIDE0
// |TreeBranchToLeafNumber\/queryOneMyData$
// |TreeBranchToLeafNumber\/delMyData$
// |TreeBranchToLeafNumber\/addMyData$
// |TreeBranchToLeafNumber\/updateMyData$
// |TreeBranchToLeafNumber\/queryList$
/**/

class TreeBranchToLeafNumberAction extends  Action {
	...
	//TreeBranchToLeafNumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchToLeafNumberModel = D("TreeBranchToLeafNumber");
				$editData = $treeBranchToLeafNumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchToLeafNumberModel = D("TreeBranchToLeafNumber");
					$sc = 	$treeBranchToLeafNumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $treeBranchToLeafNumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $treeBranchToLeafNumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $treeBranchToLeafNumberModel->where($selectWhere )->count();
		...
		$listData = $treeBranchToLeafNumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tree_branch_to_leaf_number" ;
...
;
		...
		$treeBranchToLeafNumberModel = D("TreeBranchToLeafNumber");
    	$tmpArr = $treeBranchToLeafNumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_branch_to_leaf_number".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_branch_to_leaf_number",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeBranchToLeafNumberModel = D("TreeBranchToLeafNumber");
        $editData = $treeBranchToLeafNumberModel->getById($params);
...
;
        ...
        $this->redirect("TreeBranchToLeafNumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_branch_to_leaf_numberModel = D ( "TreeBranchToLeafNumber" );
				$tmpArr = $tree_branch_to_leaf_numberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_branch_to_leaf_numberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_branch_to_leaf_numberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TreeBranchToLeafNumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
