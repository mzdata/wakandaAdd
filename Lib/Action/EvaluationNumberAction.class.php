...
//$evaluationNumberModel = D("EvaluationNumber"); //BASECIDE0
// |EvaluationNumber\/queryOneMyData$
// |EvaluationNumber\/delMyData$
// |EvaluationNumber\/addMyData$
// |EvaluationNumber\/updateMyData$
// |EvaluationNumber\/queryList$
/**/

class EvaluationNumberAction extends  Action {
	...
	//EvaluationNumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$evaluationNumberModel = D("EvaluationNumber");
				$editData = $evaluationNumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$evaluationNumberModel = D("EvaluationNumber");
					$sc = 	$evaluationNumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $evaluationNumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $evaluationNumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $evaluationNumberModel->where($selectWhere )->count();
		...
		$listData = $evaluationNumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."evaluation_number" ;
...
;
		...
		$evaluationNumberModel = D("EvaluationNumber");
    	$tmpArr = $evaluationNumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "evaluation_number".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"evaluation_number",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$evaluationNumberModel = D("EvaluationNumber");
        $editData = $evaluationNumberModel->getById($params);
...
;
        ...
        $this->redirect("EvaluationNumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$evaluation_numberModel = D ( "EvaluationNumber" );
				$tmpArr = $evaluation_numberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$evaluation_numberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $evaluation_numberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../EvaluationNumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
