...
//$skuAliasModel = D("SkuAlias"); //BASECIDE0
// |SkuAlias\/queryOneMyData$
// |SkuAlias\/delMyData$
// |SkuAlias\/addMyData$
// |SkuAlias\/updateMyData$
// |SkuAlias\/queryList$
/*
  id  id 
  tenantCode  租户编号 
...
class SkuAliasAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$skuAliasModel = D("SkuAlias");
				$editData = $skuAliasModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$skuAliasModel = D("SkuAlias");
					$sc = 	$skuAliasModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["aliasId"]))		$data["alias_id"] =    $this->addslashesx($jsonArr["aliasId"]); 
            ...
            $su = $skuAliasModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $skuAliasModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or name like '%$searchkey%' or value like '%$searchkey%' or update_time like '%$searchkey%' or available like '%$searchkey%' or alias_id like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $skuAliasModel->where($selectWhere )->count();
		...
		$listData = $skuAliasModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$skuAliasModel = D("SkuAlias");
    	$tmpArr = $skuAliasModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sku_alias".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sku_alias",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$skuAliasModel = D("SkuAlias");
        $editData = $skuAliasModel->getById($params);
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sku_aliasModel = D ( "SkuAlias" );
				$tmpArr = $sku_aliasModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sku_aliasModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sku_aliasModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
