...
//$tbCiRulelogModel = D("TbCiRulelog"); //BASECIDE0
// |TbCiRulelog\/queryOneMyData$
// |TbCiRulelog\/delMyData$
// |TbCiRulelog\/addMyData$
// |TbCiRulelog\/updateMyData$
// |TbCiRulelog\/queryList$
/*
  id  id 
  tenantCode  租户code 
  unsalableRules  滞销规则 
  commodityBlacklist  黑名单 
  effectiveDate  影响开始日期 
  createTime  创建时间 
  operator  操作人 */

class TbCiRulelogAction extends  Action {
	...
	//TbCiRulelog\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbCiRulelogModel = D("TbCiRulelog");
				$editData = $tbCiRulelogModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbCiRulelogModel = D("TbCiRulelog");
					$sc = 	$tbCiRulelogModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["unsalableRules"]))		$data["unsalable_rules"] =    $this->addslashesx($jsonArr["unsalableRules"]); 
		if(isset($jsonArr["commodityBlacklist"]))		$data["commodity_blacklist"] =    $this->addslashesx($jsonArr["commodityBlacklist"]); 
		if(isset($jsonArr["effectiveDate"]))		$data["effective_date"] =    $this->addslashesx($jsonArr["effectiveDate"]); 
		if(isset($jsonArr["createTime"]))		$data["create_time"] =    $this->addslashesx($jsonArr["createTime"]); 
		if(isset($jsonArr["operator"]))		$data["operator"] =    $this->addslashesx($jsonArr["operator"]); 

            $su = $tbCiRulelogModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["unsalable_rules"] = $this->addslashesx(  $jsonArr["UnsalableRules"]); 
				$data["commodity_blacklist"] = $this->addslashesx(  $jsonArr["CommodityBlacklist"]); 
				$data["effective_date"] = $this->addslashesx(  $jsonArr["EffectiveDate"]); 
				$data["create_time"] = $this->addslashesx(  $jsonArr["CreateTime"]); 
				$data["operator"] = $this->addslashesx(  $jsonArr["Operator"]); 



				$su =  $tbCiRulelogModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or unsalable_rules like '%$searchkey%' or commodity_blacklist like '%$searchkey%' or effective_date like '%$searchkey%' or create_time like '%$searchkey%' or operator like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tbCiRulelogModel->where($selectWhere )->count();
		...
		$listData = $tbCiRulelogModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_ci_rulelog" ;
...
;
		...
		$tbCiRulelogModel = D("TbCiRulelog");
    	$tmpArr = $tbCiRulelogModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_ci_rulelog".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_ci_rulelog",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["unsalable_rules"] = addslashes(  $_REQUEST["addData_unsalable_rules"] ); 
				$data["commodity_blacklist"] = addslashes(  $_REQUEST["addData_commodity_blacklist"] ); 
				$data["effective_date"] = addslashes(  $_REQUEST["addData_effective_date"] ); 
				$data["create_time"] = addslashes(  $_REQUEST["addData_create_time"] ); 
				$data["operator"] = addslashes(  $_REQUEST["addData_operator"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["unsalable_rules"] =  addslashes( $_REQUEST["editData_unsalable_rules"]); 
				$data["commodity_blacklist"] =  addslashes( $_REQUEST["editData_commodity_blacklist"]); 
				$data["effective_date"] =  addslashes( $_REQUEST["editData_effective_date"]); 
				$data["create_time"] =  addslashes( $_REQUEST["editData_create_time"]); 
				$data["operator"] =  addslashes( $_REQUEST["editData_operator"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbCiRulelogModel = D("TbCiRulelog");
        $editData = $tbCiRulelogModel->getById($params);
...
;
        ...
        $this->redirect("TbCiRulelog/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_ci_rulelogModel = D ( "TbCiRulelog" );
				$tmpArr = $tb_ci_rulelogModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_ci_rulelogModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_ci_rulelogModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
