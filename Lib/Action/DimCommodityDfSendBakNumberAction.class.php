...
//$dimCommodityDfSendBakNumberModel = D("DimCommodityDfSendBakNumber"); //BASECIDE0
// |DimCommodityDfSendBakNumber\/queryOneMyData$
// |DimCommodityDfSendBakNumber\/delMyData$
// |DimCommodityDfSendBakNumber\/addMyData$
// |DimCommodityDfSendBakNumber\/updateMyData$
// |DimCommodityDfSendBakNumber\/queryList$
/**/

class DimCommodityDfSendBakNumberAction extends  Action {
	...
	//DimCommodityDfSendBakNumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$dimCommodityDfSendBakNumberModel = D("DimCommodityDfSendBakNumber");
				$editData = $dimCommodityDfSendBakNumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$dimCommodityDfSendBakNumberModel = D("DimCommodityDfSendBakNumber");
					$sc = 	$dimCommodityDfSendBakNumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $dimCommodityDfSendBakNumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $dimCommodityDfSendBakNumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $dimCommodityDfSendBakNumberModel->where($selectWhere )->count();
		...
		$listData = $dimCommodityDfSendBakNumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."dim_commodity_df_send_bak_number" ;
...
;
		...
		$dimCommodityDfSendBakNumberModel = D("DimCommodityDfSendBakNumber");
    	$tmpArr = $dimCommodityDfSendBakNumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "dim_commodity_df_send_bak_number".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"dim_commodity_df_send_bak_number",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$dimCommodityDfSendBakNumberModel = D("DimCommodityDfSendBakNumber");
        $editData = $dimCommodityDfSendBakNumberModel->getById($params);
...
;
        ...
        $this->redirect("DimCommodityDfSendBakNumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$dim_commodity_df_send_bak_numberModel = D ( "DimCommodityDfSendBakNumber" );
				$tmpArr = $dim_commodity_df_send_bak_numberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$dim_commodity_df_send_bak_numberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $dim_commodity_df_send_bak_numberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../DimCommodityDfSendBakNumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
