...
//$tbDataassetsTimerangeModel = D("TbDataassetsTimerange"); //BASECIDE0
// |TbDataassetsTimerange\/queryOneMyData$
// |TbDataassetsTimerange\/delMyData$
// |TbDataassetsTimerange\/addMyData$
// |TbDataassetsTimerange\/updateMyData$
// |TbDataassetsTimerange\/queryList$
/*
  id  id 
  tenantCode  租户code 
  startDate  开始时间 
...
class TbDataassetsTimerangeAction extends  Action {
	...
	//TbDataassetsTimerange\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbDataassetsTimerangeModel = D("TbDataassetsTimerange");
				$editData = $tbDataassetsTimerangeModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbDataassetsTimerangeModel = D("TbDataassetsTimerange");
					$sc = 	$tbDataassetsTimerangeModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["startDate"]))		$data["start_date"] =    $this->addslashesx($jsonArr["startDate"]); 
		if(isset($jsonArr["endDate"]))		$data["end_date"] =    $this->addslashesx($jsonArr["endDate"]); 

            $su = $tbDataassetsTimerangeModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["start_date"] = $this->addslashesx(  $jsonArr["StartDate"]); 
				$data["end_date"] = $this->addslashesx(  $jsonArr["EndDate"]); 



				$su =  $tbDataassetsTimerangeModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or start_date like '%$searchkey%' or end_date like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tbDataassetsTimerangeModel->where($selectWhere )->count();
		...
		$listData = $tbDataassetsTimerangeModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_dataassets_timerange" ;
...
;
		...
		$tbDataassetsTimerangeModel = D("TbDataassetsTimerange");
    	$tmpArr = $tbDataassetsTimerangeModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_dataassets_timerange".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_dataassets_timerange",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["start_date"] = addslashes(  $_REQUEST["addData_start_date"] ); 
				$data["end_date"] = addslashes(  $_REQUEST["addData_end_date"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["start_date"] =  addslashes( $_REQUEST["editData_start_date"]); 
				$data["end_date"] =  addslashes( $_REQUEST["editData_end_date"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbDataassetsTimerangeModel = D("TbDataassetsTimerange");
        $editData = $tbDataassetsTimerangeModel->getById($params);
...
;
        ...
        $this->redirect("TbDataassetsTimerange/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_dataassets_timerangeModel = D ( "TbDataassetsTimerange" );
				$tmpArr = $tb_dataassets_timerangeModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_dataassets_timerangeModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_dataassets_timerangeModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TbDataassetsTimerange'>返回</a>";
			}
		} else {
			print "upload error";
		}
