...
//$testnumberModel = D("Testnumber"); //BASECIDE0
// |Testnumber\/queryOneMyData$
// |Testnumber\/delMyData$
// |Testnumber\/addMyData$
// |Testnumber\/updateMyData$
// |Testnumber\/queryList$
/**/

class TestnumberAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$testnumberModel = D("Testnumber");
				$editData = $testnumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$testnumberModel = D("Testnumber");
					$sc = 	$testnumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $testnumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $testnumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $testnumberModel->where($selectWhere )->count();
		...
		$listData = $testnumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$testnumberModel = D("Testnumber");
    	$tmpArr = $testnumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "testnumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"testnumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$testnumberModel = D("Testnumber");
        $editData = $testnumberModel->getById($params);
...
;
        ...
        $this->redirect("Testnumber/index");
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$testnumberModel->data ( $data )->save ();
