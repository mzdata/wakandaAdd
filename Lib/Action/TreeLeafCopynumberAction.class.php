...
//$treeLeafCopynumberModel = D("TreeLeafCopynumber"); //BASECIDE0
// |TreeLeafCopynumber\/queryOneMyData$
// |TreeLeafCopynumber\/delMyData$
// |TreeLeafCopynumber\/addMyData$
// |TreeLeafCopynumber\/updateMyData$
// |TreeLeafCopynumber\/queryList$
/**/

class TreeLeafCopynumberAction extends  Action {
	...
	//TreeLeafCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeLeafCopynumberModel = D("TreeLeafCopynumber");
				$editData = $treeLeafCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeLeafCopynumberModel = D("TreeLeafCopynumber");
					$sc = 	$treeLeafCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $treeLeafCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $treeLeafCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $treeLeafCopynumberModel->where($selectWhere )->count();
		...
		$listData = $treeLeafCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tree_leaf_copynumber" ;
...
;
		...
		$treeLeafCopynumberModel = D("TreeLeafCopynumber");
    	$tmpArr = $treeLeafCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_leaf_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_leaf_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeLeafCopynumberModel = D("TreeLeafCopynumber");
        $editData = $treeLeafCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("TreeLeafCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_leaf_copynumberModel = D ( "TreeLeafCopynumber" );
				$tmpArr = $tree_leaf_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_leaf_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_leaf_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TreeLeafCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
