...
//$tbCommodityInspectionModel = D("TbCommodityInspection"); //BASECIDE0
// |TbCommodityInspection\/queryOneMyData$
// |TbCommodityInspection\/delMyData$
// |TbCommodityInspection\/addMyData$
// |TbCommodityInspection\/updateMyData$
// |TbCommodityInspection\/queryList$
/*
  id  id 
  storeId  门店id 
  storeName  门店name 
...
class TbCommodityInspectionAction extends  Action {
	...
	//TbCommodityInspection\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbCommodityInspectionModel = D("TbCommodityInspection");
				$editData = $tbCommodityInspectionModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbCommodityInspectionModel = D("TbCommodityInspection");
					$sc = 	$tbCommodityInspectionModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updateTime"]))		$data["update_time"] =    $this->addslashesx($jsonArr["updateTime"]); 
            ...
            $su = $tbCommodityInspectionModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $tbCommodityInspectionModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $tbCommodityInspectionModel->where($selectWhere )->count();
		...
		$listData = $tbCommodityInspectionModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_commodity_inspection" ;
...
;
		...
		$tbCommodityInspectionModel = D("TbCommodityInspection");
    	$tmpArr = $tbCommodityInspectionModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_commodity_inspection".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_commodity_inspection",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbCommodityInspectionModel = D("TbCommodityInspection");
        $editData = $tbCommodityInspectionModel->getById($params);
...
;
        ...
        $this->redirect("TbCommodityInspection/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_commodity_inspectionModel = D ( "TbCommodityInspection" );
				$tmpArr = $tb_commodity_inspectionModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_commodity_inspectionModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_commodity_inspectionModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TbCommodityInspection'>返回</a>";
			}
		} else {
			print "upload error";
		}
