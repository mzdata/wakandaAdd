...
//$sqoopIllegalProcurementStoreModel = D("SqoopIllegalProcurementStore"); //BASECIDE0
// |SqoopIllegalProcurementStore\/queryOneMyData$
// |SqoopIllegalProcurementStore\/delMyData$
// |SqoopIllegalProcurementStore\/addMyData$
// |SqoopIllegalProcurementStore\/updateMyData$
// |SqoopIllegalProcurementStore\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  storeId  id 
...
class SqoopIllegalProcurementStoreAction extends  Action {
	...
	//SqoopIllegalProcurementStore\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqoopIllegalProcurementStoreModel = D("SqoopIllegalProcurementStore");
				$editData = $sqoopIllegalProcurementStoreModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqoopIllegalProcurementStoreModel = D("SqoopIllegalProcurementStore");
					$sc = 	$sqoopIllegalProcurementStoreModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updatedTime"]))		$data["updated_time"] =    $this->addslashesx($jsonArr["updatedTime"]); 
            ...
            $su = $sqoopIllegalProcurementStoreModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $sqoopIllegalProcurementStoreModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or store_id like '%$searchkey%' or store_name like '%$searchkey%' or updated_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $sqoopIllegalProcurementStoreModel->where($selectWhere )->count();
		...
		$listData = $sqoopIllegalProcurementStoreModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."sqoop_illegal_procurement_store" ;
...
;
		...
		$sqoopIllegalProcurementStoreModel = D("SqoopIllegalProcurementStore");
    	$tmpArr = $sqoopIllegalProcurementStoreModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sqoop_illegal_procurement_store".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sqoop_illegal_procurement_store",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$sqoopIllegalProcurementStoreModel = D("SqoopIllegalProcurementStore");
        $editData = $sqoopIllegalProcurementStoreModel->getById($params);
...
;
        ...
        $this->redirect("SqoopIllegalProcurementStore/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sqoop_illegal_procurement_storeModel = D ( "SqoopIllegalProcurementStore" );
				$tmpArr = $sqoop_illegal_procurement_storeModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sqoop_illegal_procurement_storeModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sqoop_illegal_procurement_storeModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../SqoopIllegalProcurementStore'>返回</a>";
			}
		} else {
			print "upload error";
		}
