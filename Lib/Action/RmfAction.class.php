...
class RmfAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$rmfModel = D("Rmf");
					$sc = 	$rmfModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["r1"]))		$data["r1"] =    $this->addslashesx($jsonArr["r1"]); 
		if(isset($jsonArr["r2"]))		$data["r2"] =    $this->addslashesx($jsonArr["r2"]); 
		if(isset($jsonArr["f"]))		$data["f"] =    $this->addslashesx($jsonArr["f"]); 
		if(isset($jsonArr["m"]))		$data["m"] =    $this->addslashesx($jsonArr["m"]); 
		if(isset($jsonArr["deleted"]))		$data["deleted"] =    $this->addslashesx($jsonArr["deleted"]); 

            $su = $rmfModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["r1"] = $this->addslashesx(  $jsonArr["R1"]); 
				$data["r2"] = $this->addslashesx(  $jsonArr["R2"]); 
				$data["f"] = $this->addslashesx(  $jsonArr["F"]); 
				$data["m"] = $this->addslashesx(  $jsonArr["M"]); 
				$data["deleted"] = $this->addslashesx(  $jsonArr["Deleted"]); 



				$su =  $rmfModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or tenant_name like '%$searchkey%' or r1 like '%$searchkey%' or r2 like '%$searchkey%' or f like '%$searchkey%' or m like '%$searchkey%' or deleted like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $rmfModel->where($selectWhere )->count();
		...
		$listData = $rmfModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"rmf",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["r1"] = addslashes(  $_REQUEST["addData_r1"] ); 
				$data["r2"] = addslashes(  $_REQUEST["addData_r2"] ); 
				$data["f"] = addslashes(  $_REQUEST["addData_f"] ); 
				$data["m"] = addslashes(  $_REQUEST["addData_m"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["r1"] =  addslashes( $_REQUEST["editData_r1"]); 
				$data["r2"] =  addslashes( $_REQUEST["editData_r2"]); 
				$data["f"] =  addslashes( $_REQUEST["editData_f"]); 
				$data["m"] =  addslashes( $_REQUEST["editData_m"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$rmfModel = D("Rmf");
        $editData = $rmfModel->getById($params);
