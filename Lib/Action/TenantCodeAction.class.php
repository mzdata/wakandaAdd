...
//$tenantCodeModel = D("TenantCode"); //BASECIDE0
// |TenantCode\/queryOneMyData$
// |TenantCode\/delMyData$
// |TenantCode\/addMyData$
// |TenantCode\/updateMyData$
// |TenantCode\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  option  1 */

class TenantCodeAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tenantCodeModel = D("TenantCode");
				$editData = $tenantCodeModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tenantCodeModel = D("TenantCode");
					$sc = 	$tenantCodeModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["option"]))		$data["option"] =    $this->addslashesx($jsonArr["option"]); 

            $su = $tenantCodeModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["option"] = $this->addslashesx(  $jsonArr["Option"]); 



				$su =  $tenantCodeModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or option like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tenantCodeModel->where($selectWhere )->count();
		...
		$listData = $tenantCodeModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$tenantCodeModel = D("TenantCode");
    	$tmpArr = $tenantCodeModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tenant_code".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tenant_code",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["option"] = addslashes(  $_REQUEST["addData_option"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["option"] =  addslashes( $_REQUEST["editData_option"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tenantCodeModel = D("TenantCode");
        $editData = $tenantCodeModel->getById($params);
...
;
        ...
        $this->redirect("TenantCode/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tenant_codeModel = D ( "TenantCode" );
				$tmpArr = $tenant_codeModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tenant_codeModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tenant_codeModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
