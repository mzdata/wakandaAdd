...
//$skuCategoryModel = D("SkuCategory"); //BASECIDE0
// |SkuCategory\/queryOneMyData$
// |SkuCategory\/delMyData$
// |SkuCategory\/addMyData$
// |SkuCategory\/updateMyData$
// |SkuCategory\/queryList$
/*
  id  id 
  tenantCode  租户编号 
  branchValue  枝干值 
...
class SkuCategoryAction extends  Action {
	...
	//SkuCategory\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$skuCategoryModel = D("SkuCategory");
				$editData = $skuCategoryModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$skuCategoryModel = D("SkuCategory");
					$sc = 	$skuCategoryModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["aliasId"]))		$data["alias_id"] =    $this->addslashesx($jsonArr["aliasId"]); 

            $su = $skuCategoryModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["alias_id"] = $this->addslashesx(  $jsonArr["AliasId"]); 



				$su =  $skuCategoryModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or branch_value like '%$searchkey%' or leaf_value like '%$searchkey%' or updated_time like '%$searchkey%' or alias_id like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $skuCategoryModel->where($selectWhere )->count();
		...
		$listData = $skuCategoryModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$skuCategoryModel = D("SkuCategory");
    	$tmpArr = $skuCategoryModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sku_category".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sku_category",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["alias_id"] = addslashes(  $_REQUEST["addData_alias_id"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["alias_id"] =  addslashes( $_REQUEST["editData_alias_id"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$skuCategoryModel = D("SkuCategory");
        $editData = $skuCategoryModel->getById($params);
...
;
        ...
        $this->redirect("SkuCategory/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sku_categoryModel = D ( "SkuCategory" );
				$tmpArr = $sku_categoryModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sku_categoryModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sku_categoryModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
