...
//$rmfCopynumberModel = D("RmfCopynumber"); //BASECIDE0
// |RmfCopynumber\/queryOneMyData$
// |RmfCopynumber\/delMyData$
// |RmfCopynumber\/addMyData$
// |RmfCopynumber\/updateMyData$
// |RmfCopynumber\/queryList$
/**/

class RmfCopynumberAction extends  Action {
	...
	//RmfCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$rmfCopynumberModel = D("RmfCopynumber");
				$editData = $rmfCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$rmfCopynumberModel = D("RmfCopynumber");
					$sc = 	$rmfCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $rmfCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $rmfCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $rmfCopynumberModel->where($selectWhere )->count();
		...
		$listData = $rmfCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."rmf_copynumber" ;
...
;
		...
		$rmfCopynumberModel = D("RmfCopynumber");
    	$tmpArr = $rmfCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "rmf_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"rmf_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$rmfCopynumberModel = D("RmfCopynumber");
        $editData = $rmfCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("RmfCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$rmf_copynumberModel = D ( "RmfCopynumber" );
				$tmpArr = $rmf_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$rmf_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $rmf_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../RmfCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
