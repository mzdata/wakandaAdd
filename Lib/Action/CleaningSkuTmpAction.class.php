...
//$cleaningSkuTmpModel = D("CleaningSkuTmp"); //BASECIDE0
// |CleaningSkuTmp\/queryOneMyData$
// |CleaningSkuTmp\/delMyData$
// |CleaningSkuTmp\/addMyData$
// |CleaningSkuTmp\/updateMyData$
// |CleaningSkuTmp\/queryList$
/*
  id  id 
  commodityId  名字加规格 
  tenantCode  租户编号 
...
class CleaningSkuTmpAction extends  Action {
	...
	//CleaningSkuTmp\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$cleaningSkuTmpModel = D("CleaningSkuTmp");
				$editData = $cleaningSkuTmpModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$cleaningSkuTmpModel = D("CleaningSkuTmp");
					$sc = 	$cleaningSkuTmpModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updatedTime"]))		$data["updated_time"] =    $this->addslashesx($jsonArr["updatedTime"]); 
            ...
            $su = $cleaningSkuTmpModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $cleaningSkuTmpModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $cleaningSkuTmpModel->where($selectWhere )->count();
		...
		$listData = $cleaningSkuTmpModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."cleaning_sku_tmp" ;
...
;
		...
		$cleaningSkuTmpModel = D("CleaningSkuTmp");
    	$tmpArr = $cleaningSkuTmpModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "cleaning_sku_tmp".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"cleaning_sku_tmp",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$cleaningSkuTmpModel = D("CleaningSkuTmp");
        $editData = $cleaningSkuTmpModel->getById($params);
...
;
        ...
        $this->redirect("CleaningSkuTmp/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$cleaning_sku_tmpModel = D ( "CleaningSkuTmp" );
				$tmpArr = $cleaning_sku_tmpModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$cleaning_sku_tmpModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $cleaning_sku_tmpModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../CleaningSkuTmp'>返回</a>";
			}
		} else {
			print "upload error";
		}
