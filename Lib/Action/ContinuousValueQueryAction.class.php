...
//$continuousValueQueryModel = D("ContinuousValueQuery"); //BASECIDE0
// |ContinuousValueQuery\/queryOneMyData$
// |ContinuousValueQuery\/delMyData$
// |ContinuousValueQuery\/addMyData$
// |ContinuousValueQuery\/updateMyData$
// |ContinuousValueQuery\/queryList$
/*
  id  id 
...
class ContinuousValueQueryAction extends  Action {
	...
	//ContinuousValueQuery\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$continuousValueQueryModel = D("ContinuousValueQuery");
				$editData = $continuousValueQueryModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$continuousValueQueryModel = D("ContinuousValueQuery");
					$sc = 	$continuousValueQueryModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["selected"]))		$data["selected"] =    $this->addslashesx($jsonArr["selected"]); 
            ...
            $su = $continuousValueQueryModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $continuousValueQueryModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or catalog_name like '%$searchkey%' or catalog_enum like '%$searchkey%' or metric_name like '%$searchkey%' or metric_enum like '%$searchkey%' or metric_unit like '%$searchkey%' or table_name like '%$searchkey%' or time_field like '%$searchkey%' or value_field like '%$searchkey%' or deleted like '%$searchkey%' or sort like '%$searchkey%' or selected like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $continuousValueQueryModel->where($selectWhere )->count();
		...
		$listData = $continuousValueQueryModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."continuous_value_query" ;
...
;
		...
		$continuousValueQueryModel = D("ContinuousValueQuery");
    	$tmpArr = $continuousValueQueryModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "continuous_value_query".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"continuous_value_query",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$continuousValueQueryModel = D("ContinuousValueQuery");
        $editData = $continuousValueQueryModel->getById($params);
...
;
        ...
        $this->redirect("ContinuousValueQuery/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$continuous_value_queryModel = D ( "ContinuousValueQuery" );
				$tmpArr = $continuous_value_queryModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$continuous_value_queryModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $continuous_value_queryModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../ContinuousValueQuery'>返回</a>";
			}
		} else {
			print "upload error";
		}
