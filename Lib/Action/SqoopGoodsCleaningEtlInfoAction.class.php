...
//$sqoopGoodsCleaningEtlInfoModel = D("SqoopGoodsCleaningEtlInfo"); //BASECIDE0
// |SqoopGoodsCleaningEtlInfo\/queryOneMyData$
// |SqoopGoodsCleaningEtlInfo\/delMyData$
// |SqoopGoodsCleaningEtlInfo\/addMyData$
// |SqoopGoodsCleaningEtlInfo\/updateMyData$
// |SqoopGoodsCleaningEtlInfo\/queryList$
/*
  id  id 
  time1  开始日期 
  time2  结束日期 
...
class SqoopGoodsCleaningEtlInfoAction extends  Action {
	...
	//SqoopGoodsCleaningEtlInfo\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqoopGoodsCleaningEtlInfoModel = D("SqoopGoodsCleaningEtlInfo");
				$editData = $sqoopGoodsCleaningEtlInfoModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqoopGoodsCleaningEtlInfoModel = D("SqoopGoodsCleaningEtlInfo");
					$sc = 	$sqoopGoodsCleaningEtlInfoModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["time1"]))		$data["time1"] =    $this->addslashesx($jsonArr["time1"]); 
		if(isset($jsonArr["time2"]))		$data["time2"] =    $this->addslashesx($jsonArr["time2"]); 
		if(isset($jsonArr["cnt"]))		$data["cnt"] =    $this->addslashesx($jsonArr["cnt"]); 
		...
		if(isset($jsonArr["tenantCode"]))		$data["tenant_code"] =    $this->addslashesx($jsonArr["tenantCode"]); 
            ...
            $su = $sqoopGoodsCleaningEtlInfoModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
 				...
 				$data = array();
									...
									$data["time1"] = $this->addslashesx(  $jsonArr["Time1"]); 
				$data["time2"] = $this->addslashesx(  $jsonArr["Time2"]); 
				$data["cnt"] = $this->addslashesx(  $jsonArr["Cnt"]); 
				$data["updated_time"] = $this->addslashesx(  $jsonArr["UpdatedTime"]); 
				$data["tenant_code"] = $this->addslashesx(  $jsonArr["TenantCode"]); 



				$su =  $sqoopGoodsCleaningEtlInfoModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or time1 like '%$searchkey%' or time2 like '%$searchkey%' or cnt like '%$searchkey%' or updated_time like '%$searchkey%' or tenant_code like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $sqoopGoodsCleaningEtlInfoModel->where($selectWhere )->count();
		...
		$listData = $sqoopGoodsCleaningEtlInfoModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."sqoop_goods_cleaning_etl_info" ;
...
;
		...
		$sqoopGoodsCleaningEtlInfoModel = D("SqoopGoodsCleaningEtlInfo");
    	$tmpArr = $sqoopGoodsCleaningEtlInfoModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sqoop_goods_cleaning_etl_info".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sqoop_goods_cleaning_etl_info",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["time1"] = addslashes(  $_REQUEST["addData_time1"] ); 
				$data["time2"] = addslashes(  $_REQUEST["addData_time2"] ); 
				$data["cnt"] = addslashes(  $_REQUEST["addData_cnt"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["time1"] =  addslashes( $_REQUEST["editData_time1"]); 
				$data["time2"] =  addslashes( $_REQUEST["editData_time2"]); 
				$data["cnt"] =  addslashes( $_REQUEST["editData_cnt"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$sqoopGoodsCleaningEtlInfoModel = D("SqoopGoodsCleaningEtlInfo");
        $editData = $sqoopGoodsCleaningEtlInfoModel->getById($params);
...
;
        ...
        $this->redirect("SqoopGoodsCleaningEtlInfo/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sqoop_goods_cleaning_etl_infoModel = D ( "SqoopGoodsCleaningEtlInfo" );
				$tmpArr = $sqoop_goods_cleaning_etl_infoModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sqoop_goods_cleaning_etl_infoModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sqoop_goods_cleaning_etl_infoModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../SqoopGoodsCleaningEtlInfo'>返回</a>";
			}
		} else {
			print "upload error";
		}
