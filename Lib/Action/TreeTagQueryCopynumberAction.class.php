...
//$treeTagQueryCopynumberModel = D("TreeTagQueryCopynumber"); //BASECIDE0
// |TreeTagQueryCopynumber\/queryOneMyData$
// |TreeTagQueryCopynumber\/delMyData$
// |TreeTagQueryCopynumber\/addMyData$
// |TreeTagQueryCopynumber\/updateMyData$
// |TreeTagQueryCopynumber\/queryList$
/**/

class TreeTagQueryCopynumberAction extends  Action {
	...
	//TreeTagQueryCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeTagQueryCopynumberModel = D("TreeTagQueryCopynumber");
				$editData = $treeTagQueryCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeTagQueryCopynumberModel = D("TreeTagQueryCopynumber");
					$sc = 	$treeTagQueryCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $treeTagQueryCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $treeTagQueryCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $treeTagQueryCopynumberModel->where($selectWhere )->count();
		...
		$listData = $treeTagQueryCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tree_tag_query_copynumber" ;
...
;
		...
		$treeTagQueryCopynumberModel = D("TreeTagQueryCopynumber");
    	$tmpArr = $treeTagQueryCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_tag_query_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_tag_query_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeTagQueryCopynumberModel = D("TreeTagQueryCopynumber");
        $editData = $treeTagQueryCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("TreeTagQueryCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_tag_query_copynumberModel = D ( "TreeTagQueryCopynumber" );
				$tmpArr = $tree_tag_query_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_tag_query_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_tag_query_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TreeTagQueryCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
