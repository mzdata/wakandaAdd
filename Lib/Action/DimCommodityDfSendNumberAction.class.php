...
//$dimCommodityDfSendNumberModel = D("DimCommodityDfSendNumber"); //BASECIDE0
// |DimCommodityDfSendNumber\/queryOneMyData$
// |DimCommodityDfSendNumber\/delMyData$
// |DimCommodityDfSendNumber\/addMyData$
// |DimCommodityDfSendNumber\/updateMyData$
// |DimCommodityDfSendNumber\/queryList$
/**/

class DimCommodityDfSendNumberAction extends  Action {
	...
	//DimCommodityDfSendNumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$dimCommodityDfSendNumberModel = D("DimCommodityDfSendNumber");
				$editData = $dimCommodityDfSendNumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$dimCommodityDfSendNumberModel = D("DimCommodityDfSendNumber");
					$sc = 	$dimCommodityDfSendNumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $dimCommodityDfSendNumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $dimCommodityDfSendNumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $dimCommodityDfSendNumberModel->where($selectWhere )->count();
		...
		$listData = $dimCommodityDfSendNumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."dim_commodity_df_send_number" ;
...
;
		...
		$dimCommodityDfSendNumberModel = D("DimCommodityDfSendNumber");
    	$tmpArr = $dimCommodityDfSendNumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "dim_commodity_df_send_number".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"dim_commodity_df_send_number",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$dimCommodityDfSendNumberModel = D("DimCommodityDfSendNumber");
        $editData = $dimCommodityDfSendNumberModel->getById($params);
...
;
        ...
        $this->redirect("DimCommodityDfSendNumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$dim_commodity_df_send_numberModel = D ( "DimCommodityDfSendNumber" );
				$tmpArr = $dim_commodity_df_send_numberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$dim_commodity_df_send_numberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $dim_commodity_df_send_numberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../DimCommodityDfSendNumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
