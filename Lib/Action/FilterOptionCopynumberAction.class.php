...
//$filterOptionCopynumberModel = D("FilterOptionCopynumber"); //BASECIDE0
// |FilterOptionCopynumber\/queryOneMyData$
// |FilterOptionCopynumber\/delMyData$
// |FilterOptionCopynumber\/addMyData$
// |FilterOptionCopynumber\/updateMyData$
// |FilterOptionCopynumber\/queryList$
/**/

class FilterOptionCopynumberAction extends  Action {
	...
	//FilterOptionCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterOptionCopynumberModel = D("FilterOptionCopynumber");
				$editData = $filterOptionCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterOptionCopynumberModel = D("FilterOptionCopynumber");
					$sc = 	$filterOptionCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $filterOptionCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $filterOptionCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $filterOptionCopynumberModel->where($selectWhere )->count();
		...
		$listData = $filterOptionCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."filter_option_copynumber" ;
...
;
		...
		$filterOptionCopynumberModel = D("FilterOptionCopynumber");
    	$tmpArr = $filterOptionCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "filter_option_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"filter_option_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$filterOptionCopynumberModel = D("FilterOptionCopynumber");
        $editData = $filterOptionCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("FilterOptionCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$filter_option_copynumberModel = D ( "FilterOptionCopynumber" );
				$tmpArr = $filter_option_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$filter_option_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $filter_option_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../FilterOptionCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
