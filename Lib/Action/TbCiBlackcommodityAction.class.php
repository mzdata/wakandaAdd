...
//$tbCiBlackcommodityModel = D("TbCiBlackcommodity"); //BASECIDE0
// |TbCiBlackcommodity\/queryOneMyData$
// |TbCiBlackcommodity\/delMyData$
// |TbCiBlackcommodity\/addMyData$
// |TbCiBlackcommodity\/updateMyData$
// |TbCiBlackcommodity\/queryList$
/*
  id  id 
  tenantCode  租户code 
  commodityId  商品名称 
...
class TbCiBlackcommodityAction extends  Action {
	...
	//TbCiBlackcommodity\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbCiBlackcommodityModel = D("TbCiBlackcommodity");
				$editData = $tbCiBlackcommodityModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tbCiBlackcommodityModel = D("TbCiBlackcommodity");
					$sc = 	$tbCiBlackcommodityModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["commodityName"]))		$data["commodity_name"] =    $this->addslashesx($jsonArr["commodityName"]); 
            ...
            $su = $tbCiBlackcommodityModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $tbCiBlackcommodityModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or commodity_id like '%$searchkey%' or commodity_name like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tbCiBlackcommodityModel->where($selectWhere )->count();
		...
		$listData = $tbCiBlackcommodityModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tb_ci_blackcommodity" ;
...
;
		...
		$tbCiBlackcommodityModel = D("TbCiBlackcommodity");
    	$tmpArr = $tbCiBlackcommodityModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tb_ci_blackcommodity".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tb_ci_blackcommodity",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tbCiBlackcommodityModel = D("TbCiBlackcommodity");
        $editData = $tbCiBlackcommodityModel->getById($params);
...
;
        ...
        $this->redirect("TbCiBlackcommodity/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tb_ci_blackcommodityModel = D ( "TbCiBlackcommodity" );
				$tmpArr = $tb_ci_blackcommodityModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tb_ci_blackcommodityModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tb_ci_blackcommodityModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TbCiBlackcommodity'>返回</a>";
			}
		} else {
			print "upload error";
		}
