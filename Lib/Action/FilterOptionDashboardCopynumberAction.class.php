...
//$filterOptionDashboardCopynumberModel = D("FilterOptionDashboardCopynumber"); //BASECIDE0
// |FilterOptionDashboardCopynumber\/queryOneMyData$
// |FilterOptionDashboardCopynumber\/delMyData$
// |FilterOptionDashboardCopynumber\/addMyData$
// |FilterOptionDashboardCopynumber\/updateMyData$
// |FilterOptionDashboardCopynumber\/queryList$
/**/

class FilterOptionDashboardCopynumberAction extends  Action {
	...
	//FilterOptionDashboardCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterOptionDashboardCopynumberModel = D("FilterOptionDashboardCopynumber");
				$editData = $filterOptionDashboardCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterOptionDashboardCopynumberModel = D("FilterOptionDashboardCopynumber");
					$sc = 	$filterOptionDashboardCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $filterOptionDashboardCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $filterOptionDashboardCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $filterOptionDashboardCopynumberModel->where($selectWhere )->count();
		...
		$listData = $filterOptionDashboardCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."filter_option_dashboard_copynumber" ;
...
;
		...
		$filterOptionDashboardCopynumberModel = D("FilterOptionDashboardCopynumber");
    	$tmpArr = $filterOptionDashboardCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "filter_option_dashboard_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"filter_option_dashboard_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$filterOptionDashboardCopynumberModel = D("FilterOptionDashboardCopynumber");
        $editData = $filterOptionDashboardCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("FilterOptionDashboardCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$filter_option_dashboard_copynumberModel = D ( "FilterOptionDashboardCopynumber" );
				$tmpArr = $filter_option_dashboard_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$filter_option_dashboard_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $filter_option_dashboard_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../FilterOptionDashboardCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
