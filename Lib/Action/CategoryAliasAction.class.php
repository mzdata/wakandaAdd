...
//$categoryAliasModel = D("CategoryAlias"); //BASECIDE0
// |CategoryAlias\/queryOneMyData$
// |CategoryAlias\/delMyData$
// |CategoryAlias\/addMyData$
// |CategoryAlias\/updateMyData$
// |CategoryAlias\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  name  枝干名 
...
class CategoryAliasAction extends  Action {
	...
	//CategoryAlias\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$categoryAliasModel = D("CategoryAlias");
				$editData = $categoryAliasModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$categoryAliasModel = D("CategoryAlias");
					$sc = 	$categoryAliasModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updateTime"]))		$data["update_time"] =    $this->addslashesx($jsonArr["updateTime"]); 
            ...
            $su = $categoryAliasModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $categoryAliasModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or name like '%$searchkey%' or value like '%$searchkey%' or update_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $categoryAliasModel->where($selectWhere )->count();
		...
		$listData = $categoryAliasModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."category_alias" ;
...
;
		...
		$categoryAliasModel = D("CategoryAlias");
    	$tmpArr = $categoryAliasModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "category_alias".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"category_alias",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$categoryAliasModel = D("CategoryAlias");
        $editData = $categoryAliasModel->getById($params);
...
;
        ...
        $this->redirect("CategoryAlias/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$category_aliasModel = D ( "CategoryAlias" );
				$tmpArr = $category_aliasModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$category_aliasModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $category_aliasModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../CategoryAlias'>返回</a>";
			}
		} else {
			print "upload error";
		}
