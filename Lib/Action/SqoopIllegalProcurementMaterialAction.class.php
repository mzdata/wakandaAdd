...
//$sqoopIllegalProcurementMaterialModel = D("SqoopIllegalProcurementMaterial"); //BASECIDE0
// |SqoopIllegalProcurementMaterial\/queryOneMyData$
// |SqoopIllegalProcurementMaterial\/delMyData$
// |SqoopIllegalProcurementMaterial\/addMyData$
// |SqoopIllegalProcurementMaterial\/updateMyData$
// |SqoopIllegalProcurementMaterial\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  materia  材料 
...
class SqoopIllegalProcurementMaterialAction extends  Action {
	...
	//SqoopIllegalProcurementMaterial\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqoopIllegalProcurementMaterialModel = D("SqoopIllegalProcurementMaterial");
				$editData = $sqoopIllegalProcurementMaterialModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$sqoopIllegalProcurementMaterialModel = D("SqoopIllegalProcurementMaterial");
					$sc = 	$sqoopIllegalProcurementMaterialModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["updatedTime"]))		$data["updated_time"] =    $this->addslashesx($jsonArr["updatedTime"]); 
            ...
            $su = $sqoopIllegalProcurementMaterialModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $sqoopIllegalProcurementMaterialModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or materia like '%$searchkey%' or updated_time like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $sqoopIllegalProcurementMaterialModel->where($selectWhere )->count();
		...
		$listData = $sqoopIllegalProcurementMaterialModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."sqoop_illegal_procurement_material" ;
...
;
		...
		$sqoopIllegalProcurementMaterialModel = D("SqoopIllegalProcurementMaterial");
    	$tmpArr = $sqoopIllegalProcurementMaterialModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "sqoop_illegal_procurement_material".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"sqoop_illegal_procurement_material",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$sqoopIllegalProcurementMaterialModel = D("SqoopIllegalProcurementMaterial");
        $editData = $sqoopIllegalProcurementMaterialModel->getById($params);
...
;
        ...
        $this->redirect("SqoopIllegalProcurementMaterial/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$sqoop_illegal_procurement_materialModel = D ( "SqoopIllegalProcurementMaterial" );
				$tmpArr = $sqoop_illegal_procurement_materialModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$sqoop_illegal_procurement_materialModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $sqoop_illegal_procurement_materialModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../SqoopIllegalProcurementMaterial'>返回</a>";
			}
		} else {
			print "upload error";
		}
