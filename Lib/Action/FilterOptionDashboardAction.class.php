...
//$filterOptionDashboardModel = D("FilterOptionDashboard"); //BASECIDE0
// |FilterOptionDashboard\/queryOneMyData$
// |FilterOptionDashboard\/delMyData$
// |FilterOptionDashboard\/addMyData$
// |FilterOptionDashboard\/updateMyData$
// |FilterOptionDashboard\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  tenantName  租户名称 
...
class FilterOptionDashboardAction extends  Action {
	...
	//FilterOptionDashboard\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterOptionDashboardModel = D("FilterOptionDashboard");
				$editData = $filterOptionDashboardModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterOptionDashboardModel = D("FilterOptionDashboard");
					$sc = 	$filterOptionDashboardModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["enum"]))		$data["enum"] =    $this->addslashesx($jsonArr["enum"]); 
		...
		if(isset($jsonArr["deleted"]))		$data["deleted"] =    $this->addslashesx($jsonArr["deleted"]); 
            ...
            $su = $filterOptionDashboardModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["enum"] = $this->addslashesx(  $jsonArr["Enum"]); 
				...
				$su =  $filterOptionDashboardModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or tenant_name like '%$searchkey%' or type like '%$searchkey%' or enum like '%$searchkey%' or name like '%$searchkey%' or value like '%$searchkey%' or deleted like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $filterOptionDashboardModel->where($selectWhere )->count();
		...
		$listData = $filterOptionDashboardModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."filter_option_dashboard" ;
...
;
		...
		$filterOptionDashboardModel = D("FilterOptionDashboard");
    	$tmpArr = $filterOptionDashboardModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "filter_option_dashboard".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"filter_option_dashboard",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["enum"] = addslashes(  $_REQUEST["addData_enum"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["enum"] =  addslashes( $_REQUEST["editData_enum"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$filterOptionDashboardModel = D("FilterOptionDashboard");
        $editData = $filterOptionDashboardModel->getById($params);
...
;
        ...
        $this->redirect("FilterOptionDashboard/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$filter_option_dashboardModel = D ( "FilterOptionDashboard" );
				$tmpArr = $filter_option_dashboardModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$filter_option_dashboardModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $filter_option_dashboardModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../FilterOptionDashboard'>返回</a>";
			}
		} else {
			print "upload error";
		}
