...
//$filterTagModel = D("FilterTag"); //BASECIDE0
// |FilterTag\/queryOneMyData$
// |FilterTag\/delMyData$
// |FilterTag\/addMyData$
// |FilterTag\/updateMyData$
// |FilterTag\/queryList$
/*
  id  id 
  name  指标名 
...
class FilterTagAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterTagModel = D("FilterTag");
				$editData = $filterTagModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$filterTagModel = D("FilterTag");
					$sc = 	$filterTagModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["tagEnum"]))		$data["tag_enum"] =    $this->addslashesx($jsonArr["tagEnum"]); 
		...
		if(isset($jsonArr["selected"]))		$data["selected"] =    $this->addslashesx($jsonArr["selected"]); 
            ...
            $su = $filterTagModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["tag_enum"] = $this->addslashesx(  $jsonArr["TagEnum"]); 
				...
				$su =  $filterTagModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or name like '%$searchkey%' or tag_enum like '%$searchkey%' or deleted like '%$searchkey%' or sort like '%$searchkey%' or selected like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $filterTagModel->where($selectWhere )->count();
		...
		$listData = $filterTagModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$filterTagModel = D("FilterTag");
    	$tmpArr = $filterTagModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "filter_tag".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"filter_tag",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["tag_enum"] = addslashes(  $_REQUEST["addData_tag_enum"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["tag_enum"] =  addslashes( $_REQUEST["editData_tag_enum"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$filterTagModel = D("FilterTag");
        $editData = $filterTagModel->getById($params);
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$filter_tagModel = D ( "FilterTag" );
				$tmpArr = $filter_tagModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$filter_tagModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $filter_tagModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
