...
//$treeLeafModel = D("TreeLeaf"); //BASECIDE0
// |TreeLeaf\/queryOneMyData$
// |TreeLeaf\/delMyData$
// |TreeLeaf\/addMyData$
// |TreeLeaf\/updateMyData$
// |TreeLeaf\/queryList$
/*
  id  id 
  tenantCode  租户编号 
...
class TreeLeafAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeLeafModel = D("TreeLeaf");
				$editData = $treeLeafModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeLeafModel = D("TreeLeaf");
					$sc = 	$treeLeafModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["available"]))		$data["available"] =    $this->addslashesx($jsonArr["available"]); 
            ...
            $su = $treeLeafModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $treeLeafModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or type like '%$searchkey%' or name like '%$searchkey%' or value like '%$searchkey%' or deleted like '%$searchkey%' or update_time like '%$searchkey%' or available like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $treeLeafModel->where($selectWhere )->count();
		...
		$listData = $treeLeafModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$treeLeafModel = D("TreeLeaf");
    	$tmpArr = $treeLeafModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_leaf".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_leaf",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeLeafModel = D("TreeLeaf");
        $editData = $treeLeafModel->getById($params);
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_leafModel = D ( "TreeLeaf" );
				$tmpArr = $tree_leafModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_leafModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_leafModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
