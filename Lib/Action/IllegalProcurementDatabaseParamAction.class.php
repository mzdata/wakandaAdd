...
//$illegalProcurementDatabaseParamModel = D("IllegalProcurementDatabaseParam"); //BASECIDE0
// |IllegalProcurementDatabaseParam\/queryOneMyData$
// |IllegalProcurementDatabaseParam\/delMyData$
// |IllegalProcurementDatabaseParam\/addMyData$
// |IllegalProcurementDatabaseParam\/updateMyData$
// |IllegalProcurementDatabaseParam\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  bizTag  业务类型: 
...
class IllegalProcurementDatabaseParamAction extends  Action {
	...
	//IllegalProcurementDatabaseParam\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$illegalProcurementDatabaseParamModel = D("IllegalProcurementDatabaseParam");
				$editData = $illegalProcurementDatabaseParamModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$illegalProcurementDatabaseParamModel = D("IllegalProcurementDatabaseParam");
					$sc = 	$illegalProcurementDatabaseParamModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["detail"]))		$data["detail"] =    $this->addslashesx($jsonArr["detail"]); 
		if(isset($jsonArr["updatedTime"]))		$data["updated_time"] =    $this->addslashesx($jsonArr["updatedTime"]); 
		if(isset($jsonArr["format"]))		$data["format"] =    $this->addslashesx($jsonArr["format"]); 

            $su = $illegalProcurementDatabaseParamModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["detail"] = $this->addslashesx(  $jsonArr["Detail"]); 
				$data["updated_time"] = $this->addslashesx(  $jsonArr["UpdatedTime"]); 
				$data["format"] = $this->addslashesx(  $jsonArr["Format"]); 



				$su =  $illegalProcurementDatabaseParamModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or biz_tag like '%$searchkey%' or type like '%$searchkey%' or detail like '%$searchkey%' or updated_time like '%$searchkey%' or format like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $illegalProcurementDatabaseParamModel->where($selectWhere )->count();
		...
		$listData = $illegalProcurementDatabaseParamModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."illegal_procurement_database_param" ;
...
;
		...
		$illegalProcurementDatabaseParamModel = D("IllegalProcurementDatabaseParam");
    	$tmpArr = $illegalProcurementDatabaseParamModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "illegal_procurement_database_param".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"illegal_procurement_database_param",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["detail"] = addslashes(  $_REQUEST["addData_detail"] ); 
				$data["updated_time"] = addslashes(  $_REQUEST["addData_updated_time"] ); 
				$data["format"] = addslashes(  $_REQUEST["addData_format"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["detail"] =  addslashes( $_REQUEST["editData_detail"]); 
				$data["updated_time"] =  addslashes( $_REQUEST["editData_updated_time"]); 
				$data["format"] =  addslashes( $_REQUEST["editData_format"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$illegalProcurementDatabaseParamModel = D("IllegalProcurementDatabaseParam");
        $editData = $illegalProcurementDatabaseParamModel->getById($params);
...
;
        ...
        $this->redirect("IllegalProcurementDatabaseParam/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$illegal_procurement_database_paramModel = D ( "IllegalProcurementDatabaseParam" );
				$tmpArr = $illegal_procurement_database_paramModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$illegal_procurement_database_paramModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $illegal_procurement_database_paramModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../IllegalProcurementDatabaseParam'>返回</a>";
			}
		} else {
			print "upload error";
		}
