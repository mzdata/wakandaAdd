...
//$tenantTaskCopynumberModel = D("TenantTaskCopynumber"); //BASECIDE0
// |TenantTaskCopynumber\/queryOneMyData$
// |TenantTaskCopynumber\/delMyData$
// |TenantTaskCopynumber\/addMyData$
// |TenantTaskCopynumber\/updateMyData$
// |TenantTaskCopynumber\/queryList$
/**/

class TenantTaskCopynumberAction extends  Action {
	...
	//TenantTaskCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tenantTaskCopynumberModel = D("TenantTaskCopynumber");
				$editData = $tenantTaskCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tenantTaskCopynumberModel = D("TenantTaskCopynumber");
					$sc = 	$tenantTaskCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $tenantTaskCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $tenantTaskCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $tenantTaskCopynumberModel->where($selectWhere )->count();
		...
		$listData = $tenantTaskCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tenant_task_copynumber" ;
...
;
		...
		$tenantTaskCopynumberModel = D("TenantTaskCopynumber");
    	$tmpArr = $tenantTaskCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tenant_task_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tenant_task_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tenantTaskCopynumberModel = D("TenantTaskCopynumber");
        $editData = $tenantTaskCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("TenantTaskCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tenant_task_copynumberModel = D ( "TenantTaskCopynumber" );
				$tmpArr = $tenant_task_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tenant_task_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tenant_task_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TenantTaskCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
