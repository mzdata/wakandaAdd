...
//$evaluationNcxModel = D("EvaluationNcx"); //BASECIDE0
// |EvaluationNcx\/queryOneMyData$
// |EvaluationNcx\/delMyData$
// |EvaluationNcx\/addMyData$
// |EvaluationNcx\/updateMyData$
// |EvaluationNcx\/queryList$
/*
  id  id 
  evaluationTime  评价时间 
  evaluationTags  评价标签 
  ...
  think  思考过程 
  negativeCategory  差评分类 
  negativeSeverity  差评程度 
  negativeContent  关键差评段落 
  positiveCategory  好评分类 
  positiveContent  关键好评段落 
  mentionedDishes  提到的菜品 
  mentionedDishesContent  提到的菜品的关键段落 */

class EvaluationNcxAction extends  Action {
	...
	//EvaluationNcx\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$evaluationNcxModel = D("EvaluationNcx");
				$editData = $evaluationNcxModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$evaluationNcxModel = D("EvaluationNcx");
					$sc = 	$evaluationNcxModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["negativeCategory"]))		$data["negative_category"] =    $this->addslashesx($jsonArr["negativeCategory"]); 
		if(isset($jsonArr["negativeSeverity"]))		$data["negative_severity"] =    $this->addslashesx($jsonArr["negativeSeverity"]); 
		if(isset($jsonArr["negativeContent"]))		$data["negative_content"] =    $this->addslashesx($jsonArr["negativeContent"]); 
		if(isset($jsonArr["positiveCategory"]))		$data["positive_category"] =    $this->addslashesx($jsonArr["positiveCategory"]); 
		if(isset($jsonArr["positiveContent"]))		$data["positive_content"] =    $this->addslashesx($jsonArr["positiveContent"]); 
		if(isset($jsonArr["mentionedDishes"]))		$data["mentioned_dishes"] =    $this->addslashesx($jsonArr["mentionedDishes"]); 
		if(isset($jsonArr["mentionedDishesContent"]))		$data["mentioned_dishes_content"] =    $this->addslashesx($jsonArr["mentionedDishesContent"]); 

            $su = $evaluationNcxModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["negative_category"] = $this->addslashesx(  $jsonArr["NegativeCategory"]); 
				$data["negative_severity"] = $this->addslashesx(  $jsonArr["NegativeSeverity"]); 
				$data["negative_content"] = $this->addslashesx(  $jsonArr["NegativeContent"]); 
				$data["positive_category"] = $this->addslashesx(  $jsonArr["PositiveCategory"]); 
				$data["positive_content"] = $this->addslashesx(  $jsonArr["PositiveContent"]); 
				$data["mentioned_dishes"] = $this->addslashesx(  $jsonArr["MentionedDishes"]); 
				$data["mentioned_dishes_content"] = $this->addslashesx(  $jsonArr["MentionedDishesContent"]); 



				$su =  $evaluationNcxModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or evaluation_time like '%$searchkey%' or evaluation_tags like '%$searchkey%' or evaluation_details like '%$searchkey%' or star_rating like '%$searchkey%' or taste_rating like '%$searchkey%' or environment_rating like '%$searchkey%' or service_rating like '%$searchkey%' or category_rating like '%$searchkey%' or ingredient_rating like '%$searchkey%' or cost_performance_rating like '%$searchkey%' or view_count like '%$searchkey%' or city like '%$searchkey%' or store_name like '%$searchkey%' or store_meituan_id like '%$searchkey%' or authorization_number like '%$searchkey%' or group_buying_id like '%$searchkey%' or group_buying_evaluation like '%$searchkey%' or evaluation_source like '%$searchkey%' or user_nickname like '%$searchkey%' or is_vip like '%$searchkey%' or user_level like '%$searchkey%' or reply_status like '%$searchkey%' or merchant_reply_content like '%$searchkey%' or merchant_reply_time like '%$searchkey%' or building_content like '%$searchkey%' or is_negative like '%$searchkey%' or evaluation_type like '%$searchkey%' or dine_way like '%$searchkey%' or department like '%$searchkey%' or evaluation_level like '%$searchkey%' or think like '%$searchkey%' or negative_category like '%$searchkey%' or negative_severity like '%$searchkey%' or negative_content like '%$searchkey%' or positive_category like '%$searchkey%' or positive_content like '%$searchkey%' or mentioned_dishes like '%$searchkey%' or mentioned_dishes_content like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $evaluationNcxModel->where($selectWhere )->count();
		...
		$listData = $evaluationNcxModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."evaluation_ncx" ;
...
;
		...
		$evaluationNcxModel = D("EvaluationNcx");
    	$tmpArr = $evaluationNcxModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "evaluation_ncx".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"evaluation_ncx",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["negative_category"] = addslashes(  $_REQUEST["addData_negative_category"] ); 
				$data["negative_severity"] = addslashes(  $_REQUEST["addData_negative_severity"] ); 
				$data["negative_content"] = addslashes(  $_REQUEST["addData_negative_content"] ); 
				$data["positive_category"] = addslashes(  $_REQUEST["addData_positive_category"] ); 
				$data["positive_content"] = addslashes(  $_REQUEST["addData_positive_content"] ); 
				$data["mentioned_dishes"] = addslashes(  $_REQUEST["addData_mentioned_dishes"] ); 
				$data["mentioned_dishes_content"] = addslashes(  $_REQUEST["addData_mentioned_dishes_content"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["negative_category"] =  addslashes( $_REQUEST["editData_negative_category"]); 
				$data["negative_severity"] =  addslashes( $_REQUEST["editData_negative_severity"]); 
				$data["negative_content"] =  addslashes( $_REQUEST["editData_negative_content"]); 
				$data["positive_category"] =  addslashes( $_REQUEST["editData_positive_category"]); 
				$data["positive_content"] =  addslashes( $_REQUEST["editData_positive_content"]); 
				$data["mentioned_dishes"] =  addslashes( $_REQUEST["editData_mentioned_dishes"]); 
				$data["mentioned_dishes_content"] =  addslashes( $_REQUEST["editData_mentioned_dishes_content"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$evaluationNcxModel = D("EvaluationNcx");
        $editData = $evaluationNcxModel->getById($params);
...
;
        ...
        $this->redirect("EvaluationNcx/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$evaluation_ncxModel = D ( "EvaluationNcx" );
				$tmpArr = $evaluation_ncxModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$evaluation_ncxModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $evaluation_ncxModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../EvaluationNcx'>返回</a>";
			}
		} else {
			print "upload error";
		}
