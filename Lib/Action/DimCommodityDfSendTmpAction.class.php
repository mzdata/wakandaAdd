...
//$dimCommodityDfSendTmpModel = D("DimCommodityDfSendTmp"); //BASECIDE0
// |DimCommodityDfSendTmp\/queryOneMyData$
// |DimCommodityDfSendTmp\/delMyData$
// |DimCommodityDfSendTmp\/addMyData$
// |DimCommodityDfSendTmp\/updateMyData$
// |DimCommodityDfSendTmp\/queryList$
/*
  id  id 
  tenantCode  租户编码 
  commodityId  商品id 
...
class DimCommodityDfSendTmpAction extends  Action {
	...
	//DimCommodityDfSendTmp\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$dimCommodityDfSendTmpModel = D("DimCommodityDfSendTmp");
				$editData = $dimCommodityDfSendTmpModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$dimCommodityDfSendTmpModel = D("DimCommodityDfSendTmp");
					$sc = 	$dimCommodityDfSendTmpModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["commodityCnt30day"]))		$data["commodity_cnt_30day"] =    $this->addslashesx($jsonArr["commodityCnt30day"]); 
            ...
            $su = $dimCommodityDfSendTmpModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $dimCommodityDfSendTmpModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $dimCommodityDfSendTmpModel->where($selectWhere )->count();
		...
		$listData = $dimCommodityDfSendTmpModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."dim_commodity_df_send_tmp" ;
...
;
		...
		$dimCommodityDfSendTmpModel = D("DimCommodityDfSendTmp");
    	$tmpArr = $dimCommodityDfSendTmpModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "dim_commodity_df_send_tmp".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"dim_commodity_df_send_tmp",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$dimCommodityDfSendTmpModel = D("DimCommodityDfSendTmp");
        $editData = $dimCommodityDfSendTmpModel->getById($params);
...
;
        ...
        $this->redirect("DimCommodityDfSendTmp/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$dim_commodity_df_send_tmpModel = D ( "DimCommodityDfSendTmp" );
				$tmpArr = $dim_commodity_df_send_tmpModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$dim_commodity_df_send_tmpModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $dim_commodity_df_send_tmpModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../DimCommodityDfSendTmp'>返回</a>";
			}
		} else {
			print "upload error";
		}
