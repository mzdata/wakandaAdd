...
//$treeBranchToLeafCopynumberModel = D("TreeBranchToLeafCopynumber"); //BASECIDE0
// |TreeBranchToLeafCopynumber\/queryOneMyData$
// |TreeBranchToLeafCopynumber\/delMyData$
// |TreeBranchToLeafCopynumber\/addMyData$
// |TreeBranchToLeafCopynumber\/updateMyData$
// |TreeBranchToLeafCopynumber\/queryList$
/**/

class TreeBranchToLeafCopynumberAction extends  Action {
	...
	//TreeBranchToLeafCopynumber\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchToLeafCopynumberModel = D("TreeBranchToLeafCopynumber");
				$editData = $treeBranchToLeafCopynumberModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$treeBranchToLeafCopynumberModel = D("TreeBranchToLeafCopynumber");
					$sc = 	$treeBranchToLeafCopynumberModel->where("id='$id'")->delete();
	    ...
	    public function updateMyData() {
            ...
            $su = $treeBranchToLeafCopynumberModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $treeBranchToLeafCopynumberModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $treeBranchToLeafCopynumberModel->where($selectWhere )->count();
		...
		$listData = $treeBranchToLeafCopynumberModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tree_branch_to_leaf_copynumber" ;
...
;
		...
		$treeBranchToLeafCopynumberModel = D("TreeBranchToLeafCopynumber");
    	$tmpArr = $treeBranchToLeafCopynumberModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tree_branch_to_leaf_copynumber".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tree_branch_to_leaf_copynumber",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$treeBranchToLeafCopynumberModel = D("TreeBranchToLeafCopynumber");
        $editData = $treeBranchToLeafCopynumberModel->getById($params);
...
;
        ...
        $this->redirect("TreeBranchToLeafCopynumber/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tree_branch_to_leaf_copynumberModel = D ( "TreeBranchToLeafCopynumber" );
				$tmpArr = $tree_branch_to_leaf_copynumberModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tree_branch_to_leaf_copynumberModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tree_branch_to_leaf_copynumberModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TreeBranchToLeafCopynumber'>返回</a>";
			}
		} else {
			print "upload error";
		}
