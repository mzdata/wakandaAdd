...
//$userLogModel = D("UserLog"); //BASECIDE0
// |UserLog\/queryOneMyData$
// |UserLog\/delMyData$
// |UserLog\/addMyData$
// |UserLog\/updateMyData$
// |UserLog\/queryList$
/*
  id  id 
...
class UserLogAction extends  Action {
 ...
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$userLogModel = D("UserLog");
				$editData = $userLogModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$userLogModel = D("UserLog");
					$sc = 	$userLogModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["user"]))		$data["user"] =    $this->addslashesx($jsonArr["user"]); 
		...
		if(isset($jsonArr["router"]))		$data["router"] =    $this->addslashesx($jsonArr["router"]); 
		...
		if(isset($jsonArr["detail"]))		$data["detail"] =    $this->addslashesx($jsonArr["detail"]); 
            ...
            $su = $userLogModel->data($data)->save();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or tenant_code like '%$searchkey%' or user like '%$searchkey%' or user_name like '%$searchkey%' or type like '%$searchkey%' or router like '%$searchkey%' or des like '%$searchkey%' or update_time like '%$searchkey%' or detail like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $userLogModel->where($selectWhere )->count();
		...
		$listData = $userLogModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
		...
		$userLogModel = D("UserLog");
    	$tmpArr = $userLogModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "user_log".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"user_log",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$userLogModel = D("UserLog");
        $editData = $userLogModel->getById($params);
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$user_logModel = D ( "UserLog" );
				$tmpArr = $user_logModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$user_logModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $user_logModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
