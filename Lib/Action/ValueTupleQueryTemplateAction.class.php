...
//$valueTupleQueryTemplateModel = D("ValueTupleQueryTemplate"); //BASECIDE0
// |ValueTupleQueryTemplate\/queryOneMyData$
// |ValueTupleQueryTemplate\/delMyData$
// |ValueTupleQueryTemplate\/addMyData$
// |ValueTupleQueryTemplate\/updateMyData$
// |ValueTupleQueryTemplate\/queryList$
/*
  id  id 
  metricName  指标名 
  metricEnum  指标值 
  ...
  timeOption  时间选项 
  timeField  时间字段 
  aggregationFuncName  值聚合函数 
  timeRangeMode  时间选项模式 
  deleted  是否已删除 */

class ValueTupleQueryTemplateAction extends  Action {
	...
	//ValueTupleQueryTemplate\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$valueTupleQueryTemplateModel = D("ValueTupleQueryTemplate");
				$editData = $valueTupleQueryTemplateModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$valueTupleQueryTemplateModel = D("ValueTupleQueryTemplate");
					$sc = 	$valueTupleQueryTemplateModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["timeOption"]))		$data["time_option"] =    $this->addslashesx($jsonArr["timeOption"]); 
		if(isset($jsonArr["timeField"]))		$data["time_field"] =    $this->addslashesx($jsonArr["timeField"]); 
		if(isset($jsonArr["aggregationFuncName"]))		$data["aggregation_func_name"] =    $this->addslashesx($jsonArr["aggregationFuncName"]); 
		if(isset($jsonArr["timeRangeMode"]))		$data["time_range_mode"] =    $this->addslashesx($jsonArr["timeRangeMode"]); 
		if(isset($jsonArr["deleted"]))		$data["deleted"] =    $this->addslashesx($jsonArr["deleted"]); 

            $su = $valueTupleQueryTemplateModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$data["time_option"] = $this->addslashesx(  $jsonArr["TimeOption"]); 
				$data["time_field"] = $this->addslashesx(  $jsonArr["TimeField"]); 
				$data["aggregation_func_name"] = $this->addslashesx(  $jsonArr["AggregationFuncName"]); 
				$data["time_range_mode"] = $this->addslashesx(  $jsonArr["TimeRangeMode"]); 
				$data["deleted"] = $this->addslashesx(  $jsonArr["Deleted"]); 



				$su =  $valueTupleQueryTemplateModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or metric_name like '%$searchkey%' or metric_enum like '%$searchkey%' or metric_unit like '%$searchkey%' or table_name like '%$searchkey%' or time_option like '%$searchkey%' or time_field like '%$searchkey%' or aggregation_func_name like '%$searchkey%' or time_range_mode like '%$searchkey%' or deleted like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $valueTupleQueryTemplateModel->where($selectWhere )->count();
		...
		$listData = $valueTupleQueryTemplateModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."value_tuple_query_template" ;
...
;
		...
		$valueTupleQueryTemplateModel = D("ValueTupleQueryTemplate");
    	$tmpArr = $valueTupleQueryTemplateModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "value_tuple_query_template".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"value_tuple_query_template",$oldFFArr,$FF);
...
;
			...
			if ($_REQUEST["act"] == "add") {
				...
				$data["time_option"] = addslashes(  $_REQUEST["addData_time_option"] ); 
				$data["time_field"] = addslashes(  $_REQUEST["addData_time_field"] ); 
				$data["aggregation_func_name"] = addslashes(  $_REQUEST["addData_aggregation_func_name"] ); 
				$data["time_range_mode"] = addslashes(  $_REQUEST["addData_time_range_mode"] ); 
...
;
        ...
        if ($_REQUEST["act"] == "edit") {
            ...
            $data = array();
				...
				$data["time_option"] =  addslashes( $_REQUEST["editData_time_option"]); 
				$data["time_field"] =  addslashes( $_REQUEST["editData_time_field"]); 
				$data["aggregation_func_name"] =  addslashes( $_REQUEST["editData_aggregation_func_name"]); 
				$data["time_range_mode"] =  addslashes( $_REQUEST["editData_time_range_mode"]); 
        ...
        $params                = $_REQUEST["params"];
        	...
        	$valueTupleQueryTemplateModel = D("ValueTupleQueryTemplate");
        $editData = $valueTupleQueryTemplateModel->getById($params);
...
;
        ...
        $this->redirect("ValueTupleQueryTemplate/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$value_tuple_query_templateModel = D ( "ValueTupleQueryTemplate" );
				$tmpArr = $value_tuple_query_templateModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$value_tuple_query_templateModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $value_tuple_query_templateModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../ValueTupleQueryTemplate'>返回</a>";
			}
		} else {
			print "upload error";
		}
