...
//$tmpCommodityNewModel = D("TmpCommodityNew"); //BASECIDE0
// |TmpCommodityNew\/queryOneMyData$
// |TmpCommodityNew\/delMyData$
// |TmpCommodityNew\/addMyData$
// |TmpCommodityNew\/updateMyData$
// |TmpCommodityNew\/queryList$
/*
  commodityId  commodity_id 
  commodityName  commodity_name */

class TmpCommodityNewAction extends  Action {
	...
	//TmpCommodityNew\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tmpCommodityNewModel = D("TmpCommodityNew");
				$editData = $tmpCommodityNewModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$tmpCommodityNewModel = D("TmpCommodityNew");
					$sc = 	$tmpCommodityNewModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["commodityName"]))		$data["commodity_name"] =    $this->addslashesx($jsonArr["commodityName"]); 
            ...
            $su = $tmpCommodityNewModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $tmpCommodityNewModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $selectWhere = " id like '%$searchkey%'  or commodity_id like '%$searchkey%' or commodity_name like '%$searchkey%'";
		if(empty($searchkey))
		{
			$selectWhere = "1=1";
		} 
        ...
        $count = $tmpCommodityNewModel->where($selectWhere )->count();
		...
		$listData = $tmpCommodityNewModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."tmp_commodity_new" ;
...
;
		...
		$tmpCommodityNewModel = D("TmpCommodityNew");
    	$tmpArr = $tmpCommodityNewModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "tmp_commodity_new".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"tmp_commodity_new",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$tmpCommodityNewModel = D("TmpCommodityNew");
        $editData = $tmpCommodityNewModel->getById($params);
...
;
        ...
        $this->redirect("TmpCommodityNew/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$tmp_commodity_newModel = D ( "TmpCommodityNew" );
				$tmpArr = $tmp_commodity_newModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$tmp_commodity_newModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $tmp_commodity_newModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../TmpCommodityNew'>返回</a>";
			}
		} else {
			print "upload error";
		}
