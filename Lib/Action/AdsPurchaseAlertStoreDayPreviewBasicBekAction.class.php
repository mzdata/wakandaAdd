...
//$adsPurchaseAlertStoreDayPreviewBasicBekModel = D("AdsPurchaseAlertStoreDayPreviewBasicBek"); //BASECIDE0
// |AdsPurchaseAlertStoreDayPreviewBasicBek\/queryOneMyData$
// |AdsPurchaseAlertStoreDayPreviewBasicBek\/delMyData$
// |AdsPurchaseAlertStoreDayPreviewBasicBek\/addMyData$
// |AdsPurchaseAlertStoreDayPreviewBasicBek\/updateMyData$
// |AdsPurchaseAlertStoreDayPreviewBasicBek\/queryList$
/*
  id  id 
  tenantCode  租户code 
  taskId  task_id 
...
class AdsPurchaseAlertStoreDayPreviewBasicBekAction extends  Action {
	...
	//AdsPurchaseAlertStoreDayPreviewBasicBek\/getActionTableName$
 function getActionTableName($tablename)
 {
     $ActionTableName            = "";
		...
		return   $ActionTableName ;
 	    ...
 	    public function queryOneMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$adsPurchaseAlertStoreDayPreviewBasicBekModel = D("AdsPurchaseAlertStoreDayPreviewBasicBek");
				$editData = $adsPurchaseAlertStoreDayPreviewBasicBekModel->getById($id);
	    ...
	    public function delMyData() {
			...
			$id=$jsonArr["id"];
					   	...
					   	$adsPurchaseAlertStoreDayPreviewBasicBekModel = D("AdsPurchaseAlertStoreDayPreviewBasicBek");
					$sc = 	$adsPurchaseAlertStoreDayPreviewBasicBekModel->where("id='$id'")->delete();
		...
		if(isset($jsonArr["dt"]))		$data["dt"] =    $this->addslashesx($jsonArr["dt"]); 
            ...
            $su = $adsPurchaseAlertStoreDayPreviewBasicBekModel->data($data)->save();
	    ...
	    public function addMyData() {
			...
			$jsonArr = json_decode($json,true);  
				...
				$su =  $adsPurchaseAlertStoreDayPreviewBasicBekModel->data($data)->add();
    ...
    public function queryList() {
        ...
        $count = $adsPurchaseAlertStoreDayPreviewBasicBekModel->where($selectWhere )->count();
		...
		$listData = $adsPurchaseAlertStoreDayPreviewBasicBekModel->where(  $selectWhere)->order("id desc")->limit($page->firstRow.','.$page->listRows)->select();
...
;
        ...
        //是否有表查看权限
        $idName = "tablename."."ads_purchase_alert_store_day_preview_basic_bek" ;
...
;
		...
		$adsPurchaseAlertStoreDayPreviewBasicBekModel = D("AdsPurchaseAlertStoreDayPreviewBasicBek");
    	$tmpArr = $adsPurchaseAlertStoreDayPreviewBasicBekModel ->getDbFields()  ;
		$showFieldsArr = array();
		foreach($tmpArr as $field)
		{
			$idName =  "ads_purchase_alert_store_day_preview_basic_bek".".".$field;
		...
		if($FFSearch==1)
	 	{
	 			$FF="";
			  	...
			  	$selectWhere = $this->getFFSearch($FFArr ,$_REQUEST,"ads_purchase_alert_store_day_preview_basic_bek",$oldFFArr,$FF);
...
;
        ...
        $params                = $_REQUEST["params"];
        	...
        	$adsPurchaseAlertStoreDayPreviewBasicBekModel = D("AdsPurchaseAlertStoreDayPreviewBasicBek");
        $editData = $adsPurchaseAlertStoreDayPreviewBasicBekModel->getById($params);
...
;
        ...
        $this->redirect("AdsPurchaseAlertStoreDayPreviewBasicBek/index");
	...
	public function upServerModel() {
		...
		if (isset ( $_FILES ["import"] ) && ($_FILES ["import"] ["error"] == 0)) {
			...
			if ($result ["error"] == 1) {
				...
				$ads_purchase_alert_store_day_preview_basic_bekModel = D ( "AdsPurchaseAlertStoreDayPreviewBasicBek" );
				$tmpArr = $ads_purchase_alert_store_day_preview_basic_bekModel->getDbFields ();
				...
				foreach ( $execl_data as $k => $v ) {
					...
					if ($k > 2) {
						...
						if (isset ( $data ["id"] ) && $data ["id"] > 0) { // update
							...
							$ads_purchase_alert_store_day_preview_basic_bekModel->data ( $data )->save ();

							$sucessNum ++;
						} else { // insert

							$su = $ads_purchase_alert_store_day_preview_basic_bekModel->data ( $data )->add ();
							if ($su) {
								$sucessNum ++;
							}
						}
				...
				print "<a href='../AdsPurchaseAlertStoreDayPreviewBasicBek'>返回</a>";
			}
		} else {
			print "upload error";
		}
